...
\begin{definition}
  A solution $w^\star \in S$ to the equation
  \begin{equation}
...
\begin{equation}
  w^{(k+1)} = w^{(k)} + \eta^{(k)} g^{(k)}(w^{(k)}),
\end{equation}
  where $\{\eta^{(k)}\}_{k=1}^\infty$ is called step size and $\{g^{(k)}\}_{k=1}^\infty$ is a sequence of descent directions is called \emph{one-step descent method}.
\end{definition}
...
\begin{equation}  
  w^{(k+1)} = w^{(k)} - \eta^{(k)} \nabla f(w^{(k)}).
\end{equation}
It is easy to see that we have ${g^{(k)}}^T \nabla f(w^{(k)}) = -|| f(w^{(k)}) ||^2 < 0$ as long as the iterates have not converged to a stationary point. This iterative scheme is called \emph{gradient descent method} and forms the basis for many of the iterative schemes used in deep learning practice. 

Notice that the computation of each iterate requires the computation of the gradient $\nabla f$. For functions of the form $f(w) = \frac{1}{n} \sum_{i=1}^n f_i(w)$ this means computing each $\nabla f_i$ individually. Remember that in deep learning settings $f_i$ represents the loss with respect to one data point $(x_i, y_i)$. When the number of data points is large, computing the full gradient $\nabla f$ becomes prohibitively expensive. 
Instead of computing the full gradient a sample $\frac{1}{b}\sum \nabla f$ is used. This introduces stochasticity to the optimization process. 
Other schemes,\dots
