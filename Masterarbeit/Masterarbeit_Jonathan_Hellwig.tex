...
In \autoref{sec:smdedl}, we summarize the works by Wu et al.\ \cite{wuNoisyGradientDescent2020a} and Li et al.\ \cite{liValidityModelingSGD2021} that describe methods to simulate SMDEs. We perform a series of experiments to validate the findings of Li et al.\ \cite{liValidityModelingSGD2021} and extend the scope of the investigation by examining different model classes.  
In \autoref{sec:applications}, we outline one application of SMDE in improving the performance and stability of deep learning algorithms found in \cite{liValidityModelingSGD2021}.
...
\begin{definition}[Expected risk, empirical risk, \protect{ \cite{bottouOptimizationMethodsLargeScale2018}}]
  ...
  \min_{w \in \R^d} R(w) = \ev{\ell(M(X;w),Y)}
\end{equation}
\emph{expected risk}, where $X,Y$ are $\mathcal{X}$-valued and $\mathcal{Y}$-valued random variables respectively. Further, for a set of samples $\{(x_i, y_i)\}_{i=1}^n \subset (\mathcal{X} \times  \mathcal{Y})^n$ we call
\begin{equation}
...
\end{proof}
We present the result from \cite[pp.~149]{eAppliedStochasticAnalysis2021} as definition because in the next subsection we introduce a more general existence result from \cite{liStochasticModifiedEquations2019}.
...
\label{sec:sde_model}
In this section, we introduce a continuous-time model for SGD developed by Li et al.\ in \cite{liStochasticModifiedEquations2017}. First, we consider the non-deterministic case of GD and derive an ODE system to model the limiting behavior of GD following a framework found in \cite{hairerGeometricNumericalIntegration2013}. Then, we introduce a stochastic generalization of the model building on the results of \autoref{sec:BackgroundSDETheory}. Lastly, we discuss the rigorous framework by Li et al.\ to show the convergence of SGD and the continuous-time model in a weak sense and expand on some details of their proofs.
...
\begin{example}[Linear regression]
  ...
  \begin{figure}[htb]
    ...
    \caption{Mean square loss function for the linear regression problem with $n=1000$ samples and weight $w=4$ and bias $b=1$.}
...
Next, we introduce a framework taken from Hairer et al.\ \cite{hairerGeometricNumericalIntegration2013} and slightly modify it such that it matches the notation used for stochastic modified equations in \autoref{sec:smde_sgd} and impose some additional conditions.

\begin{definition}[Modified ordinary differential equation, \protect{\cite[pp.~337]{hairerGeometricNumericalIntegration2013}}]
  Let $\mathbf{a}: \R^d \rightarrow \R^d$ be a function with continuous derivatives of up to order $r \in \N$. Let $w:[0,T] \rightarrow \R^d$ be the solution of the differential equation
  \begin{equation*}
...
\begin{example}[\protect{\autoref{ex:linear_regression}} continued]
  ...
  \begin{figure}[htb]
    ...
    \caption{Error between GD iterates and the solution of first and second order modified equation for linear regression problem with $n=1000$ samples, dimension $d=5$ and final time $T=1$ for descreasing learning rates $\eta$.}
...
\end{equation}
where $\{w_k^{w,0}: k = 0,1,\dots, N\}$ denotes the iterates of SGD defined in equation \eqref{eq:stochastic_gradient_descent} with $w_0 = w$ and $\{\widetilde{W}_k^{w,0} = W_{k\eta}: k = 0,1,\dots, N\}$ the solution of the SDE defined in equation \eqref{eq:general_sde} with initial value $W_0 = w$ at the time steps corresponding to the SGD iterates.
...
The MNIST \cite{dengMNISTDatabaseHandwritten2012} dataset contains grayscale images of handwritten digits with a size of 28x28 pixels. It is made up of 60000 training images and 10000 test images.
...
\begin{table}[h!]
  \begin{center}
    ...
    \begin{tabular}{l|c|c|r}
      ...
      \multirow{2}{*}{1} & Conv2d & in\_channels $\in \{16,32,64\}$, out\_channels $\in \{16,32,64\}$ \\
      & & kernel\_size=3, stride=1 & \\
       & Batch2d & num\_features=3 & ReLU\\
      \multirow{2}{*}{2} & Conv2d & in\_channels $\in \{16,32,64\}$, out\_channels $\in \{16,32,64\}$ \\
      & & kernel\_size=3, stride=1 & \\
    \end{tabular}
  \end{center}
\end{table}
...
\end{definition}
Li et al.\ \cite{liValidityModelingSGD2021} construct the SVAG iterates such that for $w \in \R^d$ we have
\begin{align*}
...
\end{align*}
This means that the gradients of the amplified function have the same expectation as the gradients of original objective function, but their covariance matrix scaled by a factor of $l \in \N$. We note that for the case of $l=1$ the SVAG iterates coincide with the SGD iterates.
Li et al.\ \cite{liValidityModelingSGD2021} show that the SVAG iterates approximate the first order SMDE as $l \in \N$ grows under strong smoothness assumption. In the proof of the following theorem, they use similar techniques as presented in the proof of \autoref{thm:second_order}.
...
\end{proof}
The conditions of \autoref{thm:svag} are more restrictive than the conditions for the approximation results of the first order SMDE  presented in \autoref{cor:first_order}. Li et al.\ \cite{liValidityModelingSGD2021} claim that by using similar mollification techniques as in the proof of \autoref{thm:second_order} the assumptions of \autoref{thm:svag} can be relaxed.

\subsection{Experiments}
\label{subsec:experiments}
In this section, we simulate the SMDE using SVAG for the three models presented in \autoref{subsec:deep_learning_architectures}: a fully connected network on MNIST \cite{dengMNISTDatabaseHandwritten2012}, PreResNet-32 \cite{heDeepResidualLearning2016a} and AlexNet \cite{krizhevskyImageNetClassificationDeep2012} on CIFAR-10 \cite{krizhevskyLearningMultipleLayers2009}. We follow the approach by Li et al.\ \cite{liValidityModelingSGD2021} and use the SVAG iterates for increasing values of the scaling factor $l=1,2,4,8,16$ to simulate the SMDE solution. We implement the SVAG algorithm using PyTorch \cite{paszkePyTorchImperativeStyle2019} version 1.13.0 and run our experiments on GoogleColab \cite{bisongGoogleColaboratory2019} using a standard GPU runtime. 
...
\subsubsection{Fully connected network on MNIST}
...
For the PreResNet-32 \cite{heIdentityMappingsDeep2016} architecture we conducted two experiments with different choices of initial learning rate $\eta$ and batch size $B$. In \autoref{fig:preresnet_128_012}, we show loss values and classifications accuracies for both the validation and training dataset for initial learning rate $\eta = 0.12$ and batch size $B=128$. We observe that the SGD trajectory mostly coincides with the SVAG iterates for $l=2,4,8,16$. The validation loss shows a small difference in its limiting value.
In \autoref{fig:preresnet1024_lr096}, we have the same experimental setup up with $\eta=0.96$ and $B=1024$. We observe that the training accuracy of SGD iterates grows more slowly compared to the SVAG iterates. Further, the validation accuracy shows differing limiting behavior.
...
We note that from the experiments conducted we cannot conclude that the SMDE model fails in these cases. First, the loss functions do not fulfill the assumptions of the SVAG approximation result in \autoref{thm:svag}. Further, both the validation and training accuracy are not smooth. This means that we cannot apply \autoref{thm:svag}. Moreover, the result only implies approximation in a distributional sense. That means it is not sufficient to look at single trajectories as we did in our experiments.
All in all, we were able to reproduce the results of Li et al.\ \cite{liValidityModelingSGD2021} for the PreResNet-32 architecture and conducted new experiments with a fully connected network and AlexNet \cite{krizhevskyImageNetClassificationDeep2012}. However, following our discussion above these results are not sufficient to determine the validity of the application of the first order SMDE to deep learning objectives.

\section{Applications}
\label{sec:applications}
In this section, we cover an application of first order SMDE. We derive the linear scaling rule \cite{goyalAccurateLargeMinibatch2018, krizhevskyOneWeirdTrick2014} which asserts that when increasing the batch size $B \in \N$ by a factor of $m > 0$, then the dynamics of SGD are preserved if the learning rate $\eta > 0$ is scaled by the same factor. Li et al.\ \cite{liValidityModelingSGD2021} assert that the linear scaling rule follows from the first order SMDE, but do not state how the relation can be derived. 
...
In this thesis, we set out to understand the theoretical properties of SGD for deep learning objectives. We started with the works of Orvieto et al.\ \cite{orvietoContinuoustimeModelsStochastic2019a} and Li et al.\ \cite{liValidityModelingSGD2021} that discuss the properties of the SMDE model and its validity in the context of deep learning. In order to give some context to the discussion of the SMDE model, in \autoref{sec:stochastic_optimization} we discussed convergence results under restrictive convexity and smoothness assumptions. This discussion gives some insight into the behavior of SGD for overparametrized models, but the assumptions are too restrictive for deep learning objectives. In \autoref{sec:BackgroundSDETheory}, we presented some basic definitions and properties of stochastic differential equations. In \autoref{sec:sde_model}, we introduced the well-known concept of modified equations and applied it to gradient descent to motivate stochastic modified differential equations introduced by Li et al.\ \cite{liStochasticModifiedEquations2019}. Further, we presented an example by Li et al.\ \cite{liStochasticModifiedEquations2019}, in which we could solve the SMDE analytically and gain some insight about the dynamics of SGD. In \autoref{sec:smdedl}, we applied the SVAG algorithm to three neural network models and compared the results with SGD and reproduced the results by Li et al.\ and covered some new cases. In \autoref{sec:applications}, we showed that the linear scaling rule can be derived with the first SMDEs.
...
In the recent years, there is a rich body of literature building on the initial work of Li et al.\ \cite{liStochasticModifiedEquations2017} and applications like the linear scaling rule show that SMDE are a promising approach to understand the dynamics of stochastic optimization algorithms.
