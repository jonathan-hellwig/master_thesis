...
\begin{proof}
  ...
  Using \autoref{eq:continuity} we have
  \begin{equation}
    \begin{split}
...
\begin{definition}[Euler's method]
  Let $0 \geq t_0 < t_1 < \dots < t_N = T, \Delta t_n = t_{n+1} - t_n$. Then, the time-discrete scheme $\{w_n\}_{n=1}^N$ given by
  \begin{equation}
...
\begin{theorem}[\autocite{eAppliedStochasticAnalysis2021}]
  ...
  \end{equation}
  and \emph{Itô isometry}
  \begin{equation}
    \E \left[ \int_S^T f(\omega,t)dB_t \right]^2 = \E \left[ \int_S^T f^2(\omega,t)dt \right].
  \end{equation}
\end{theorem}
Consider the case where $\{B_t\}_{t\geq0}$ is an $m$-dimensional Brownian motion and $\pmb{f},\pmb{g}  \in \R^{d \times m}$, we have
\begin{equation}
  \begin{split}
...
\begin{theorem}[Itô's formula]
  ...
  \begin{equation}
    \begin{split}
    Y_t = f(X_0) &+ \int_0^t \langle b, \nabla f \rangle+ \frac{1}{2}Tr\left[\sigma \nabla^2 f \sigma^T\right]ds \\
    &+ \int_0^t \nabla f^T \sigma dW_s.
    \end{split}
  \end{equation}
\end{theorem}
...
\begin{example}[Ornstein-Uhlenbeck process]
  ...
  \begin{equation}
    X_t = e^{-\gamma t}x + \sigma \int_0^t e^{-\gamma (t - s) B_s}.
  \end{equation}
\end{example}
...
\begin{example}[Linear regression]
  ...
  \end{equation}
  % In \hyperref[fig:quadratic_loss_function]{figure~\ref}
...
\end{equation}
for $\gamma ~ N(0,I)$. The total objective is 
\begin{equation}
  \label{eq:objective_function}
  f(w) = \mathbb{E} f_{\gamma}(w) = \frac{1}{2} w^T H w.
\end{equation}
...
\end{equation}
Substituting \autoref{eq:ornstein_solution} into \autoref{eq:objective_function} we obtain
\begin{equation}
  \begin{split}
...
\begin{equation}
  ...
  \end{split}
\end{equation}
Now, using \autoref{eq:multivariate_ito_isometry}, we follow
\begin{equation}
  \begin{split}
