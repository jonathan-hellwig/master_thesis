...
\begin{definition}[Euler-Maruyama scheme, \protect{\cite[pp.~305]{kloedenNumericalSolutionStochastic2013}}]
  Let $0 \leq t_0 < t_1 < \dots < t_N = T$, $\Delta t_k = t_{k+1} - t_k$ and let $\Delta B_k \sim \CN(0,\Delta t_k)$. Then, the time-discrete scheme $\{W_k\}_{k=0}^N$ given by
\begin{equation}
  \label{eq:euler_maruyama}
  W_{k+1} = W_k + \mathbf{a}(W_k, t_k)\Delta t + \mathbf{b}(W_k, t_k) \Delta B_k
\end{equation}
