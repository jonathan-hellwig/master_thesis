...
\begin{definition}[Time discrete approximation;]
  A time discrete approximation $\{w_k\}_{k=1}^N$ with step size $h$ is a right continuous process with left-hand limits. The approximation $w_{n,h} = w_h(t_n)$ is $\mathcal{F}_{t_n}$ measurable w.r.t. a time discretization $\mathcal{T}^M_h$ and recursively defined by a function $\psi$ such that for $n=0,1,\dots,N-1$ holds
  \begin{equation}
    w_{n+1, h} = \Phi(w_{0,h}, \dots, w_{n,h}, t_0, \dots, t_n, Z^1_n,\dots, Z_n^k)
  \end{equation}
  for some finite number $k$ of $\mathcal{F}_{t_{n+1}}$ measurable random variables $Z^j_n, 1 \leq j \leq k$.
\end{definition}
\begin{definition}
  A time discrete approximation $w$ with maximum step size $h$ \emph{converges strongly} to $W$ at time $T$ as $h \rightarrow 0$ if 
  \begin{equation}
    \lim_{h \rightarrow 0} \mathbb{E}(\norm{W_{T} - \widehat{W}(T)}) = 0.
  \end{equation}
  The time discrete approximation $\{w_k\}_{k=1}^N$ converges strongly with order $p>0$ to the time-continuous process $\{W_t\}_{t \geq 0}$ at time $T$ as $h \rightarrow 0$ if there exists a constant $K > 0$, which does not depend on $h$, and a $\delta_0 > 0$ such that 
  \begin{equation}
    \mathbb{E}(\norm{W_T - \widehat{W}(T)}) \leq K h^p
  \end{equation}
  holds for each $h \in ]0, \delta_0[$.
\end{definition}

\begin{definition}
  A time discrete approximation $\widehat{W}$ converges weakly to $W$ at time $T$ as $h \rightarrow 0$ with respect to a class $\mathcal{C}$ of test functions $f: \R^n \rightarrow \R$ if 
  \begin{equation}
    \lim_{h \rightarrow 0} |\mathbb{E}(f(W_T)) - \mathbb{E}(f(\widehat{W}(T)))| = 0
  \end{equation}
  holds for all $f \in \mathcal{C}$.
  A time discrete approximation $\widehat{W}$ converges weakly with order $p$ to $W$ at time $T$ as $h \rightarrow 0$ if for each $f \in \CC^{2(p+1)}(\R^n, \R)$ there exists a constant $K_f$ and a finite $\delta_0$ such that 
  \begin{equation}
     |\mathbb{E}(f(W_T)) - \mathbb{E}(f(\widehat{W}(T)))| = K_f h^p
  \end{equation}
  for each $h \in ]0, \delta_0[$.
\end{definition}
\begin{definition}[Euler-Maruyama scheme, \protect{\cite[pp.~305]{kloedenNumericalSolutionStochastic2013}}]
  Let $0 \leq t_0 < t_1 < \dots < t_N = T$, $\Delta t_k = t_{k+1} - t_k$ and let $Z_k \sim \CN(0,\Delta t_k)$. Then, the time-discrete scheme $\{W_k\}_{k=0}^N$ given by
\begin{equation}
  \label{eq:euler_maruyama}
  W_{k+1} = W_k + \mathbf{a}(W_k, t_k)\Delta t_k + \mathbf{b}(W_k, t_k) Z_k
\end{equation}
...
\begin{definition}[Modified equation, \protect{\cite[pp.~337]{hairerGeometricNumericalIntegration2013}}]
  ...
  \begin{equation*}
    \widetilde{w}'(t) = \mathbf{a}^h(\widetilde{w}(t)), \quad \widetilde{w}(0) = w_0
  \end{equation*}
  where $\mathbf{a}^h(\widetilde{w}) \defeq \mathbf{a}_0(\widetilde{w}) + hb_1(\widetilde{w}) + h^2\mathbf{a}_2(\widetilde{w})+\cdots + h^r\mathbf{a}_r(\widetilde{w})$
...
\begin{align*}
  \widetilde{w}(t+h) &= \widetilde{w}(t) + \mathbf{a}^h(\widetilde{w}(t))h + \frac{1}{2}\mathbf{a}^h(\widetilde{w}(t))^\T \nabla \mathbf{a}^h(\widetilde{w}(t))h^2 + \CO(h^3) \\
  &= \widetilde{w}(t) + (\mathbf{a}_0(\widetilde{w}(t)) + \mathbf{a}_1(\widetilde{w}(t))h + \cdots)h \\
  &+ \frac{1}{2}(\mathbf{a}_0(\widetilde{w}(t)) + \mathbf{a}_1(\widetilde{w}(t))h + \cdots)^\T (\nabla \mathbf{a}_0(\widetilde{w}(t)) + \nabla \mathbf{a}_1(\widetilde{w}(t))h + \cdots)h^2 + \CO(h^3).
\end{align*}
Rearranging by the powers of $h$ yields
\begin{equation}
  \label{eq:modified_equation_powers}
  \widetilde{w}(t+h) = \widetilde{w}(t) + \mathbf{a}_0(\widetilde{w}(t))h + \left( \frac{1}{2}\mathbf{a}_0(\widetilde{w}(t))^\T \nabla \mathbf{a}_0(\widetilde{w}(t)) + b_1(\widetilde{w}(t))\right)h^2 + \CO(h^3). 
\end{equation}
...
\begin{equation*}
    ...
    \frac{1}{2}\mathbf{a}_0(\widetilde{w}(t))^\T \nabla \mathbf{a}_0(\widetilde{w}(t)) + \mathbf{a}_1(\widetilde{w}(t)) = 0.
\end{equation*}
...
\begin{definition}[Stochatic modified differential equation]
  ...
  \begin{equation*}
    dW_t = -\nabla f(W_t)dt + (\eta \Sigma(W_t))^{\frac{1}{2}}d\mathbf{B}_t
  \end{equation*}
...
\begin{equation}
  ...
  dW^{\eta, \epsilon}_t = \mathbf{a}(W^{\eta, \epsilon}_t, \eta, \epsilon)dt + \sqrt{\eta}\mathbf{b}(W^{\eta, \epsilon}_t, \eta, \epsilon)d\mathbf{B}_t, \quad W_0 = w_0, \quad t \in [0,T],
\end{equation}
...
\begin{equation*}
  \mathbf{a}(w,\eta, \epsilon) = \mathbf{a}_0(w, \epsilon) + \eta \mathbf{a}_1(w, \epsilon), \quad \mathbf{b}(w,\eta, \epsilon) s= \mathbf{b}_0(w, \epsilon).
\end{equation*}
...
\begin{lemma}[\protect{\cite{liStochasticModifiedEquations2019}}]
  ...
  We define $ \widetilde{\Delta}(w) \defeq \widetilde{W}^{w,0}_1 - w$. Suppose that $\mathbf{a}_0, \mathbf{a}_1, \mathbf{b} \in G^3$. Then we have
  \begin{enumerate}[label=(\roman*)]
    \item  $\ev{\widetilde{\Delta}_{(i)}(w)} = a_{0}(w,\epsilon)_{(i)}\eta + \left[\frac{1}{2}\sum_{j=1}^d a_{0}(w,\epsilon)_{(j)}\partial_{(j)}a_0(w,\epsilon)_{(i)} + a_1(w,\epsilon)_{(i)}\right] \eta^2 + \mathcal{O}(\eta^3)$, $i,j= 1,\dots,d$,
...
\begin{proof}
  ...
  \begin{equation*}
    \mathbf{a}_0(w, \epsilon) = - \moll * \nabla f(w), \quad \mathbf{a}_1(w, \epsilon) = -\frac{1}{4}\moll * (\nabla\norm{\nabla f(w)}^2), \quad\mathbf{b}_0(w, \epsilon) = \moll * \Sigma(w)^{\frac{1}{2}}.
  \end{equation*}
  Next, we show that $\mathbf{a}_0 + \eta \mathbf{a}_1$, $\sigma_0$ satisfy a Lipschitz condition in $w \in \R^d$ uniformly in $\eta, \epsilon$.
  ...
  \end{equation*}
  Thus, we have that $\mathbf{a}_0 + \eta \mathbf{a}_1$, $\sigma_0$ are uniformly Lipschitz. 
  ...
  We note that we have $f \in G^4_w \subset \CL^1_{\text{loc}}(\R^d)$. Thus, we have $\nabla f, \nabla \norm{\nabla f}^2, \Sigma^{\frac{1}{2}} \in \CL^1_{\text{loc}}(\R^d)$. By \autoref{lemma:mollifiers} we obtain smoothness for $\mathbf{a}_0, \mathbf{a}_1$ and $\mathbf{b}_0$. Further, polynomial growth is satisfied since for $\psi \in G$ and a multiindex $J$ we have
  \begin{align*}
...
\begin{equation*}
  dW_t = -\nabla f(W_t) + \sqrt{\frac{\eta}{B}}\Sigma^\frac{1}{2}(W_t)dB_t.
\end{equation*}
\section{Conclusion}
We can see 
