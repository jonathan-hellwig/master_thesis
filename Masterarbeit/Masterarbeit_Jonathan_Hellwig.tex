...
\begin{equation}
  \min_{w \in \mathbb{R}^d} R(w) = \mathbb{E}[\ell(m_{w}(x),y)].
\end{equation}
...
\begin{definition}[Local and global minimizer]
  ...
  \begin{equation}
    ...
    f(w^\star) \leq f(w)
  \end{equation} 
  for all $w \in \mathbb{R}^d$. A value $w^\star \in \mathbb{R}^d$ is called \emph{local minimizer} if there exists an $\epsilon > 0$ such that \ref{eq:minimizer} holds for all $w \in \mathbb{R}^d, ||w-w^\star|| < \epsilon$.
\end{definition}
...
We say the sequence  $\{W_k\}_{k=0}^{\infty}$ converges to $W$ \emph{in distribution} if
  \begin{equation}
    \lim_{k \rightarrow \infty}\E [f(W_k) - f(W)] = 0
  \end{equation}
...
\begin{align*}
  \lim_{k \rightarrow \infty} \E [f(W_k) - f(W)] &= \lim_{k \rightarrow \infty}\int_{\Omega} f(W_k) - f(W) d\BP \\
...
\begin{proof}
  ...
  \begin{align}
    ...
    \int_0^1 (\nabla f(w_t) - \nabla f(w))^T(v-w)dt &\leq \int_0^1 L ||w_t-w||_2 ||v-w||_2dt \\
    &= L ||v-w||_2 \int_0^1 t dt = \frac{L}{2} ||v-w||_2
  \end{align}
...
Now, we present a notion of convexity that is present in the convergence literature of SGD \autocite{sebbouhAlmostSureConvergence2021}.
...
In recent years, there have been developments that build on the initial converge results from Robbins et al.\ \autocite{robbinsStochasticApproximationMethod1951}. There are results for almost sure convergence of the iterates \autocite{zhouStochasticMirrorDescent2017}\autocite{nguyenSGDHogwildConvergence2018}\autocite{sebbouhAlmostSureConvergence2021}, convergence of the function values and convergence of the gradients in expectation \autocite{bottouOptimizationMethodsLargeScale2018}.  
...
\begin{lemma}
  ...
  \begin{equation}
    \E [||\nabla f_{\gamma}(w)||^2_2] \leq 4 \CL (f(w) - f(w^\star)) + 2 \sigma^2,
  \end{equation}
  where $\sigma^2 \defeq \sup_{w \in \R^d} \E [||\nabla f(w)||^2]$ and $\CL \defeq \sup_{\gamma} L_{\gamma}$.
\end{lemma}
\begin{proof}
  ...
  \begin{align*}
    \E [||\nabla f_{\gamma}(w)||^2_2] &= \E [||(\nabla f_{\gamma}(w) - \nabla f_{\gamma}(w^\star)) + \nabla f_{\gamma}(w^\star)||^2_2] \\
    &\leq 2( \E [||\nabla f_{\gamma}(w) - f_{\gamma}(w^\star)||^2 ] + \E [||\nabla f_{\gamma}(w^\star)||^2]) \\
...
\begin{lemma}
  ...
  \begin{equation*}
    \E_k[||w^{(k+1)} - w^\star||] + \eta_k (f(w^{(k)}) - f^\star)) \leq ||w^{(k)} - w^\star||^2 + 2{\eta_k}^2\sigma^2,
  \end{equation*}
  for $k=1,2,\dots$.
\end{lemma}
\begin{proof}
  ...
  \begin{align*}
    ||w^{(k+1)} - w^\star||^2 &= ||w^{(k)} - w^\star - \eta_k \nabla f_{\gamma_k}(w^{(k)})||^2 \\
    &= ||w^{(k)} - w^\star||^2 + 2 \eta_k \langle \nabla f_{\gamma_k}(w^{(k)}), w^\star - w^{(k)}\rangle + {\eta_k}^2||\nabla f_{\gamma_k}(w^{(k)})||^2.
  \end{align*}
  Now using Lemma \ref{lemma:gradient_inequality} and Lemma \ref{lemma:convexity} we obtain
  \begin{align*}
    \E_k[||w^{(k+1)} - w^\star||^2] &= ||w^{(k)} - w^\star||^2 + 2 \eta_k \langle \nabla f(w^{(k)}), w^\star - w^{(k)}\rangle + {\eta_k}^2\E_k[||\nabla f_{\gamma_k}(w^{(k)})||^2] \\
    &\leq ||w^{(k)} - w^\star||^2 + 2 \eta_k(2\CL \eta_k - 1)(f(w^{(k)}) - f(w^\star))+ {\eta_k}^22\sigma^2 \\
  \end{align*}
  Now, using $\eta_k \leq \frac{1}{4 \CL}$, we have
  \begin{equation*}
    \E_k[||w^{(k+1)} - w^\star||^2] \leq ||w^{(k)} - w^\star||^2 - \eta_k(f(w^{(k)}) - f(w^\star))+ {\eta_k}^22\sigma^2.
  \end{equation*}
\end{proof}
...
\begin{theorem}
  ...
  \begin{equation*}
    \E [f(\widetilde{w^{(k)}}) - f(w^\star)] \leq \frac{||w^{(0)} - w^\star||^2}{\sum_{i=0}^{k-1}\eta_i} + 2 \sigma^2 \frac{\sum_{i=0}^{k-1}\eta_i^2}{\sum_{i=0}^{k-1}\eta_i},
  \end{equation*}
  for $k = 1, 2, \dots$.
\end{theorem}
\begin{proof}
  ...
  \begin{equation*}
    \sum_{t=0}^{k-1}\E[||w^{(t+1)} - w^\star||] + \sum_{t=0}^{k-1} \eta_t \E[f(w^{(t)}) - f^\star)] \leq \sum_{t=0}^{k-1} \E[||w^{(t)} - w^\star||^2] + \sum_{t=0}^{k-1} 2{\eta_t}^2\sigma^2.
  \end{equation*}
  Rearranging we have
  \begin{equation*}
    \sum_{t=0}^{k-1} \eta_t \E[f(w^{(t)}) - f^\star)] \leq \E[||w^{(0)} - w^\star||^2] - \E[||w^{(k+1)} - w^\star||^2] + \sigma^2\sum_{t=0}^{k-1} 2{\eta_t}^2.
  \end{equation*}
  Now, normalizing with the sum of the learning rates, we use Jensen's inequality to obtain
  \begin{equation*}
    \E[f(\widetilde{w^{(t)}}) - f^\star)] \leq \sum_{t=0}^{k-1} \frac{\eta_t}{\sum_{i=0}^{k-1}\eta_i} \E[f(w^{(t)}) - f^\star)] \leq \frac{\E[||w^{(0)} - w^\star||^2]}{\sum_{i=0}^{k-1}\eta_i} + \frac{\sigma^2\sum_{t=0}^{k-1} 2{\eta_t}^2}{\sum_{i=0}^{k-1}\eta_i}.
  \end{equation*}
\end{proof}
...
\begin{equation*}
  \E [f(\widetilde{w^{(k)}}) - f(w^\star)] \leq \frac{||w^{(0)} - w^\star||^2}{\eta k} + 2 \sigma^2 \eta.
\end{equation*}
...
\begin{lemma}
  ...
  \begin{equation*}
    \E[V_{t+1}|\CF_t] + U_{t+1} \leq V_t + Z_t.
  \end{equation*}
...
In this section, we introduce the concept of stochastic differential equations (SDEs). First, we define a new notion of integration: Ito integration. Then, by reformulating an ordinary differential equation (ODE) as an integral equation, we generalize ODEs to stochastic processes by building on Ito integrals. Next, we discuss the existence and uniqueness of solution to SDEs. Finally, we introduce a framework for approximating solutions to SDEs numerically.
\subsection{Ito Integral}
\label{subsec:ItoIntegral}
We briefly introduce the notion of Ito integral. Note that further details on the derivation can be found in \autocite{eAppliedStochasticAnalysis2021}. 
...
\end{equation*}
The rigorous construction of the Ito integral requires the definition on piece wise constant functions. Then, it can be shown that a general class of functions can be approximated by these functions. This establishes the existence of the integrals as a limit.
\begin{definition}[\autocite{eAppliedStochasticAnalysis2021}]
...
\begin{lemma}[\autocite{eAppliedStochasticAnalysis2021}]
  For $f \in \CV[S,T]$, the Ito integral satisfies
  \begin{equation}
