...
\label{sec:smdedl}
In this section, we apply stochastic modified equations to deep learning models. First, we present the MNIST \cite{dengMNISTDatabaseHandwritten2012} and the CIFAR-10 \cite{krizhevskyLearningMultipleLayers2009} datasets we use in our experiments. Secondly, we discuss common techniques in the contruction of neural network models and look at three specific neural network architectures: fully connected neural networks \cite{aggarwalNeuralNetworksDeep2018}, AlexNet \cite{krizhevskyImageNetClassificationDeep2012} and PreResNet \cite{heIdentityMappingsDeep2016}.
Thirdly, we discuss the SVAG algorithm \cite{liValidityModelingSGD2021} which allows us to simulate the solution of the SDE model efficiently. Lastly, we investigate whether the SDE model holds empirically.
...
\begin{definition}[Activation function]
  ...
  \begin{itemize}
    ...
    \item \emph{Logisitic activation}: $\sigma : \R \rightarrow (0,1), \quad \sigma(x) = \frac{1}{1+e^{-x}}$.
    \item \emph{Rectified linear unit}: $\ReLU : \R \rightarrow [0,\infty), \quad \ReLU(x) = \max\{x,0\}$.
  \end{itemize}
  A special case is the \emph{Softmax activation} which maps a vector valued input to the space of probability vectors $\mathcal{P}^K$. It is given by $\softmax:\R^m \rightarrow \R^m$, $\softmax(z)$ = $e^z / \sum_{i=1}^n e^z_i$, where $e^z \defeq (e^{z_1}, \dots, e^{z_m})^\T$ is applied to each component of $z \in \R^m$.
\end{definition}
For both the MNIST and the CIFAR-10 dataset we want to find a model that takes an image and predicts its corresponding label with high accuracy. This type of problem is called \emph{classification problem}. To train a model for a classification problem we define the loss function:
...
\begin{definition}[Feed foward neural network, weight matrix, bias \protect{\cite[pp.~19]{aggarwalNeuralNetworksDeep2018}}]
  ...
  % \end{equation*}
  for $k = 1, \dots, L$ is called \emph{feed forward neural network}. The matrices $W_1, \dots, W_{L}$ are called \emph{weights} and the vectors $b_1, \dots, b_{L}$ are called \emph{biases}. For $k=1,\dots,L+1$ we call $z_k$ in equation \eqref{eq:nn} \emph{layer}. The layer $z_1$ is called \emph{input layer}, the layers $z_k$ for $k=2,\dots,L$ are called \emph{hidden layers} and $z_{L+1}$ is called \emph{output layer}.
  The scalar activation functions in \eqref{eq:nn} are applied to each component of its input.
\end{definition}

\subsubsection{Datasets}
The MNIST \cite{dengMNISTDatabaseHandwritten2012} dataset contains grayscale images of size 28x28 of handwritten digits and their labels. It is made up of 60000 training images and 10000 testing images.

The CIFAR-10 \cite{krizhevskyLearningMultipleLayers2009} dataset contains images with three color channels of size 34x34 of ten different objects: airplanes, cars, birds, cats, deer, dogs, frogs, horses, ships and trucks. The dataset is made up of 50000 training and 10000 test images.
...
In our experiments we investigate a special case of a feed forward network which does not impose any special structure on its weight matrices. We use a network with three hidden layers with ReLU activations and ouput layer with softmax activation. Let $W_1 \in \R^{512 \times 784}$, $W_2, W_3 \in \R^{512 \times 512}$, $W_4 \in \R^{10\times 512}$  and $b_1, b_2, b_3 \in \R^{512},b_4 \in \R^{10}$. Then the model is given by
\begin{multline*}
  M(x;W_1,\dots,W_4, b_1, \dots, b_4) = z_5, \quad
  z_5 = \softmax(W_4z_4+b_4), 
  z_4 = \ReLU(W_3z_3+b_3), \\
  z_3 = \ReLU(W_2z_2+b_2), \quad
  z_2 = \ReLU(W_1x + b_1).
\end{multline*}
% Because we do not impose any structure on the weight matrices $W_1, \dots, W_4$, the network is called \emph{fully connected neural network}.
\subsubsection{AlexNet}
AlexNet \cite{krizhevskyImageNetClassificationDeep2012} is a feed forward network neural network with some extensions that exploit the special structure of images. In deep learning tasks in which the input data $x \in \R^{m_1}$ follows grid-like structure (e.g. image data, audio data) \emph{convolutional neural networks} (CNN) employ convolution operations that make use of the locality of data. On image classification tasks CNNs perform better than fully connected neural networks (CITATIONS). 
A convolutional operation for a 2-dimensional input $I : \Z \times \Z \rightarrow \R$ and a kernel $F : \Z \times \Z \rightarrow \R$ is defined by
\begin{equation*}
  O(i,j) = (F*I)(i,j) = \sum_{u,v \in \Z}I(i-u,j-v)F(u,v),
\end{equation*}
for $i,j \in \Z$. The kernel $F$ is usually a small quadratic matrix. The width of that matrix is called \emph{kernel size}. There are different strategies to define the value of the convolution ouput $O$ at the edge of an image. A \emph{valid convolution} only reduces the support of its input. On the other hand, a \emph{same convolution} pads the input image with zeros, such that output $O$ has the same support as its input $I$.

CNNs also usually contain \emph{pooling layers}. A pooling layer reduces the dimensionality of its input. Two examples of pooling are \emph{average} and \emph{max pooling}. 
% For an input $I \Z \times \Z \rightarrow \R$ the output of average pooling is given by
% \begin{equation*}
%   O(i,j) = \frac{1}{r}\sum_{}
% \end{equation*}
AlexNet \cite{krizhevskyImageNetClassificationDeep2012} consists of a featrue extracting block consisting of convolutional and max pooling layers and a classifier consiting of linear layers.
In \autoref{tab:alexnet_architecture}, we show the structure of AlexNet for the CIFAR-10 dataset we used in our experiments. We adopt the naming scheme of PyTorch \cite{paszkePyTorchImperativeStyle2019} version 1.13.1.
\begin{table}[h!]
  \begin{center}
    ...
    \begin{tabular}{l|c|c|r}
      ...
      \hline
      1 & Conv2d & in\_channels=3, out\_channels=64, kernel\_size=4, stride=2 & ReLU\\
      2 & MaxPool2d & kernel\_size=3, stride = 2\\
      3 & Conv2d & in\_channels=64, out\_channels=64, kernel\_size = 4, stride=2 & ReLU\\
      4 & MaxPool2d & kernel\_size = 3, stride = 2\\
      5 & Conv2d & in\_channels=64, out\_channels=64, kernel\_size = 4, stride=2 & ReLU\\
      6 & Conv2d & in\_channels=64, out\_channels=64, kernel\_size = 4, stride=2 & ReLU\\
      7 & Conv2d & in\_channels=64, out\_channels=64, kernel\_size = 4, stride=2 & ReLU\\
...
He et al.\ \cite{heIdentityMappingsDeep2016} introduce the PreResNet architecture that contains convolutional and poolings layers, but builds on two new innovations: Shortcut connections and batch normalization.
They observe that there is a performance dropoff for CNNs with a large number of layers. By using shortcut connections that feed the output of previous layers directly into the next they improve the performance of large networks. In particular,  for an input $x \in \R^m$ they propose to use blocks of the following form 
\begin{equation*}
  y = G(x) + x,
\end{equation*}
where $G:\R^m \rightarrow \R^m$ is a mapping that consits of either two or three convolutional layers with ReLU activation.
...
\end{equation*}
In \autoref{tab:preresnet_basic_block}
\begin{table}[h!]
  \begin{center}
    \caption{PreResNet blck}
    ...
    \begin{tabular}{l|c|c|r}
      ...
      2 & Batch2d & num\_features=3 & ReLU\\
      3 & Conv2d & in\_channels $\in \{16,32,64\}$ out\_channels $\in \{16,32,64\}$, kernel\_size=3, stride=1 & \\
      3 & Batch2d & num\_features=3 & ReLU\\
      4 & Conv2d & in\_channels $\in \{16,32,64\}$ out\_channels $\in \{16,32,64\}$, kernel\_size=3, stride=1 & \\
    \end{tabular}
  \end{center}
\end{table}
...
% \begin{table}[h!]
%   \begin{center}
%     \caption{Basic PreResNet block}
%     \label{tab:preresnet_architecture}
%     \begin{tabular}{l|c|p|r}
%       \textbf{Layer} & \textbf{Type} & \textbf{Parameters} & \textbf{Activation}\\
%       \hline
%       1 & Conv2d & in\_channels=3, out\_channels=16, kernel\_size=3, stride=1 & \\
...
\begin{figure}[htb]
  \centering
  \includegraphics[width=\textwidth]{../code/plots/resnet_svag.pdf}
...
\begin{figure}[htb]
  \centering
  \includegraphics[width=\textwidth]{../code/plots/fully_connected.pdf}
...
% We observe that the SGD iterates and SVAG iterates approach the same limiting distribution. However, in \autoref{} for a learning rate of $\eta = 6.4$ and a batch size of $B = 1024$ we observe a different behavior.
