...
Finally, in \autoref{sec:applications} we study some of the insights we can gain from the dynamics of the continuous model.
...
\begin{enumerate}[resume, label=(\roman*)]
  ...
  is finite.
\end{enumerate}
Further, the space $\CL(0,T;\CL^2(\Omega))$ consists of all measureable functions $W_{\cdot} : [0,T] \rightarrow \CL^2(\Omega)$ with $\lVert W_{\cdot} \rVert_{\CL(0,T;\CL^2(\Omega))} \defeq \int_0^T \lVert W_t \rVert_{\CL^2(\Omega)}dt = \int_0^T \ev{\norm{W_t}^2}dt < \infty$.
...
Note that almost sure convergence implies convergence in distribution. Let $f \in C_b(\R^n, \R)$ and let $\Omega_{\text{conv}} = \{\omega \in \Omega : \lim_{n \rightarrow \infty}W_n(\omega) = W(\omega)\}$ Then, by the dominated convergence theorem we have 
\begin{align*}
...
In this section we introduce fundamental concepts for stochastic processes and reference literature for further details. This introduction follows E. et al \cite{eAppliedStochasticAnalysis2021}. We define three concept that the fundamental construct of stochastic processes: the probability space.
...
\end{equation}
We see that GD with a fixed finite learning rate leads to a gradient flow for the loss function $\widetilde{f}(w) = f(w) + \norm{\nabla f(w)}^2$. The second term induces an implicit regularization to the gradients of $f$ (REFORMULATE, CITATION).
