...
\begin{center}
  \includegraphics[height=2.3cm]{pics/TUHH_logo_cropped}
  \hfill
  \includegraphics[height=2.3cm]{pics/MATH_de}
\end{center}
\begin{center}
  \includegraphics[height=2.3cm]{pics/UHH_logo_cropped}
\end{center}
\vspace*{5em}
...
\begin{center}
  ...
  {\Large
    ...
    Jonathan Hellwig\\
    from Hamburg\\
    Matriculation number: 7381194\\
    Study programm: Industrial Mathematics\\
  }
\end{center}
...
\begin{tabbing}
  ...
  Second examiner:  \= Prof. Dr. Matthias Schulte\kill
  First examiner: \> Dr. Jens-Peter M. Zemke\\
  Second examiner:  \> Prof. Dr. Matthias Schulte\\
  Advisor:     \> Dr. Jens-Peter M. Zemke\\
\end{tabbing}
\newpage
...
\begin{center}
  Hamburg, 27.12.2022
  \hfill
  \includegraphics[height=2cm]{pics/unterschrift}
\end{center}
\vspace*{-3em}
\begin{tabbing}
...
In \autoref{sec:BackgroundSDETheory}, we give an introduction to stochastic differential equations following textbooks by E et al.\ \cite{eAppliedStochasticAnalysis2021} and Kloeden et al.\ \cite{kloedenNumericalSolutionStochastic2013}. Subsequently, we introduce the well-known gradient flow model and frame it as a modified differential equation following \cite{hairerGeometricNumericalIntegration2013, smithOriginImplicitRegularization2021}. We derive the first and second order modified equations of a linear regression model and analyze its dynamics.
...
In \autoref{sec:smdedl}, we summarize the works by Wu et al.\ \cite{wuNoisyGradientDescent2020a} and Li et al.\ \cite{liValidityModelingSGD2021a} that describe methods to simulate SMDEs. We perform a series of experiments to validate the findings of Li et al.\ \cite{liValidityModelingSGD2021a} and extend the scope of the investigation by examining different model classes.  
In \autoref{sec:applications}, we outline one application of SMDE in improving the performance and stability of deep learning algorithms found in \cite{liValidityModelingSGD2021a}.
...
\begin{equation*}
  \nabla^{\alpha} f = \partial^{\alpha_1}_{x_1^{\alpha_1}}\dots\partial^{\alpha_n}_{x_n^{\alpha_n}}\ f
\end{equation*}
...
\begin{equation*}
  \nabla f(w)_{i} = \partial_{w_i} f(w)  
\end{equation*}
 for $w \in \R^d$ and $i=1,\dots,d$. Further, if $f$ is twice continuously differentiable, we use $H_f : \R^d \rightarrow \R^{d \times d}$ to denote the \emph{Hessian} which maps a $w \in \R^d$ to a matrix with the entries 
 \begin{equation*}
  H_f(w)_{i,j} = \partial_{w_i}\partial_{w_j} f(w)
 \end{equation*}
...
For a probability space $(\Omega, \CF, \BP)$ (see \autoref{subsec:stochastic_processes}) we use $\CL^p(\Omega, \CF, \BP)$ to denote the \emph{Lebesgue space} (see, e.g., \cite[pp.~636]{evansPartialDifferentialEquations1998})  for $p \in (1,\infty)$ and use $\CL^p(\Omega)$ if the $\sigma$-algebra $\CF$ and probability measure $\BP$ are clear. If we have $f \in\CL^p(\Omega, \CF, \BP)$, then
...
\end{equation*}
for all $z \in \R^d$. 
We write $\mathcal{B}(x,r)\defeq \{y \in \R^d: \norm{y-x} \leq r \}$ to denote the ball with center $x \in \R^d$ and radius $r > 0$.
...
In this section, we introduce stochastic processes and risk minimization problems in the context of deep learning applications. Next, we present results that characterize minimizers and follow with a discussion of gradient descent and its stochastic variant. In the final parts of this section, we give an overview of assumptions for the convergence analysis of SGD and extend the proofs of one particular analysis from Sebbouh et al.\ \cite{sebbouhAlmostSureConvergence2021} to give insight into the proof strategy.
 ...
 \begin{definition}[Random variable, stochastic independence, \protect{\cite[pp.~12]{eAppliedStochasticAnalysis2021}}]
  For a probability space $(\Omega, \CF, \BP)$ a mapping $W : \Omega \rightarrow \R^d$ is called \emph{random variable}, if it is measurable, i.e., for all $A \in \mathcal{R}^d$ we have $W^{-1}(A) \in \CF$.
 ...
 \end{definition}
 Throughout this thesis we need the concept of expected value and covariance matrix which characterize a probability distribution. 
 \begin{definition}[Expected value, covariance matrix, \protect{\cite[pp.~9]{eAppliedStochasticAnalysis2021}}]
  ...
  \end{equation*}
  and its \emph{covariance matrix} is defined as 
  \begin{equation*}
    \Cov (W) = \ev{(W - \ev{W}) (W - \ev{W})^\T }.
  \end{equation*}
 \end{definition}
 ...
 \begin{example}[\protect{\cite[pp.~11]{eAppliedStochasticAnalysis2021}}]
  ...
  \end{equation*}
  where $S \in \R^{d \times d}$ is a symmetric, positive definite matrix (see \autoref{def:pos_neg_def}) and $\mu \in \R^d$. We write $W \sim \CN(\mu, S)$ to denote a normally distributed random variable with parameters $S \in \R^{d \times d}$, $\mu \in \R^d$.
 \end{example}
...
% \end{equation}
We present the well-known risk minimization problem (see, e.g., \cite{bottouOptimizationMethodsLargeScale2018}) that is fundamental to train deep learning models. We use the risk minimization formulation to motivate the construction of SGD in \autoref{subsec:gd_sgd} and look at concrete examples in the experiments of \autoref{sec:smdedl}. Now, we start by formally defining models and loss functions.
...
\begin{definition}[Expected risk, empirical risk, \protect{ \cite{bottouOptimizationMethodsLargeScale2018}}]
  ...
  \min_{w \in \R^d} R(w) = \ev{\ell(M(X;w),Y)}
\end{equation}
\emph{expected risk}, where $X,Y$ are $\mathcal{X}$-valued and $\mathcal{Y}$-valued random variables respectively. Further, for a set of samples $\{(x_i, y_i)\}_{i=1}^n \subset (\mathcal{X} \times  \mathcal{Y})$ we call
\begin{equation}
...
\begin{definition}[Local and global minimizer, \protect{\cite[pp.~12]{nocedalNumericalOptimization2006}}]
  ...
  \begin{equation}
    ...
    f(w^\star) (<) \leq  \; f(w)
  \end{equation} 
  for all $w \in \R^d$. A value $w^\star \in \R^d$ is called (strict) \emph{local minimizer} if there exists an $\epsilon > 0$ such that equation \eqref{eq:minimizer} holds for all $w \in \R^d$ with $\norm{w-w^\star} < \epsilon$.
\end{definition}
...
\end{proof}
This first theorem is the foundation for all commonly used optimization techniques in deep learning. The gradient can be  efficiently calculated with a procedure called backpropagation which makes use of the recursive structure of neural networks (see, e.g., \cite[pp.~200]{goodfellowDeepLearning2016}). 
...
\begin{proof}
  ...
  Using equations \eqref{eq:descent_direction} and \eqref{eq:continuity} we have
  \begin{equation*}
    \begin{split}
...
\begin{definition}[One-step method, gradient descent, step size, learning rate \protect{\cite{nocedalNumericalOptimization2006}}]
  For descent directions $\{g_k\}_{k=1}^\infty$, an initial value $w_{0}\in \R^d$ and $k=0,1,\dots$ a sequence defined by
\begin{equation*}
...
\begin{definition}[Stochastic gradient descent, \protect{\cite{polyakMethodsSpeedingConvergence1964}}]
 ...
 Let $(\Omega, \CF, \BP)$ be a probability space. For a given initial value $w_{0} \in \R^d$, a sequence of step sizes $\{\eta_k\}_{k=0}^\infty$ and i.i.d.\ random variables $\{\gamma_{k}\}_{k=0}^\infty$ defined on $(\Omega, \CF, \BP)$ the stochastic process defined by
  ...
  \end{equation}
  for $k=0,1,\dots$ is called \emph{stochastic gradient descent}.
...
Note that while introducing stochasticity to the optimization process allows for increased computational speed, it makes its analysis more difficult. In fact, for the analysis of stochastic methods it is not sufficient to only consider the expected value $\ev{\nabla f_{\gamma}(w)}$ of the gradients. As we will see in the following sections, the covariance matrix of the gradients $\Sigma(w) = \Cov(\nabla f_\gamma(w)) = \ev{(\nabla f_{\gamma}(w) - \nabla f(w)){(\nabla f_{\gamma}(w) - \nabla f(w))}^T}$ influences the dynamics of the loss function nontrivially.
...
\begin{proof}
  ...
  \begin{equation}
    ...
    \end{split}
  \end{equation}
  Combining equations \eqref{eq:Lsmoothproof1} and \eqref{eq:Lsmoothproof2} we obtain
  \begin{equation*}
    f(w) \leq f(v) + \langle \nabla f(w), v - w \rangle + \frac{L}{2} \norm{v - w}^2. \qedhere
  \end{equation*}
\end{proof}
...
Note that for continuously differentiable functions \autoref{lemma:convexity} immediately implies that convexity follows from strong convexity.
 ...
 Additionally, Bottou et al.\ \cite{bottouOptimizationMethodsLargeScale2018} present two assumptions for the SGD iterates: a bound from below for the target function on the SGD iterates and a growth condition for the second moment of the gradients.
\begin{assumption}[\protect{\cite{bottouOptimizationMethodsLargeScale2018}}]
  \label{as:sgd_convergence}
  Let $f : \R^d \rightarrow \R$ be a continuously differentiable function and let $\{w_k\}_{k=0}^\infty$ be the iterates given by SGD in equation \eqref{eq:stochastic_gradient_descent}. Then they satisfy the following conditions:
  \begin{enumerate}[label=(\roman*)]
...
In the following, we examine an analysis by Sebbouh et al.\ \autocite{sebbouhAlmostSureConvergence2021} that provides insight into the assumptions and techniques necessary to prove convergence.
...
\end{lemma}
\begin{proof}
  Let $w, w^\star \in \R^d$, where $w^\star$ is a minimizer of $f$. We use an inequality from the textbook by Nesterov \cite[pp.~67, (2.1.10)]{nesterovLecturesConvexOptimization2018} which states that for the $L$-smooth function $f_\gamma$ we have
  \begin{equation*}
  ...
  \end{align*}
\end{proof}
We now present the following result from Sebbouh et al.\ \autocite{sebbouhAlmostSureConvergence2021} which relates the distances to the minimizer $w^\star$ for two consecutive iterates.
\begin{lemma}[\protect{\cite{sebbouhAlmostSureConvergence2021}}]
  \label{lemma:sgd_iterates}
  Let $\gamma : \Omega \rightarrow \{1,\dots,n\}$ be a random variable defined on a probability space $(\Omega, \CF, \BP)$ and let $f_{\gamma} : \R^d \rightarrow \R$ be a continuously differentiable function that is $L_{\gamma}$-smooth and convex. Further, let $\{w_{k}\}_{k=0}^{\infty}$ be the iterates of SGD defined in equation \eqref{eq:stochastic_gradient_descent} and let $\eta_k \leq \frac{1}{4 \CL}$ for $k=1,2,\dots$. Then, we have
  \begin{equation*}
    \E[\norm{w_{k+1} - w^\star}^2|w_k] + \eta_k (f(w_{k}) - f(w^\star))) \leq \norm{w_{k} - w^\star}^2 + 2{\eta_k}^2\sigma^2,
  \end{equation*}
  for $k=1,2,\dots$.
\end{lemma}
\begin{proof}
  ...
  where $\nabla \mathbf{f}(w) = \left(\nabla f_1(w), \dots, \nabla f_n(w)\right)$ following an approach by Wu et al.\ \cite{wuNoisyGradientDescent2020a}. Using the properties of the conditional expectation found in \autoref{thm:conditional_expectation} we have
  \begin{multline*}
  ...
  We take the conditional expectation on both sides of equation \eqref{eq:iterate_norm} and obtain
  \begin{equation*}
...
\begin{theorem}[\protect{\cite{sebbouhAlmostSureConvergence2021}}]
  \label{thm:SGD_bound}
  Let $\gamma : \Omega \rightarrow \{1,\dots,n\}$ be a random variable defined on a probability space $(\Omega, \CF, \BP)$ and let $f_{\gamma} : \R^d \rightarrow \R$ be a continuously differentiable function that is both $L_{\gamma}$-smooth and convex. Then, for the iterates of SGD defined in equation \eqref{eq:stochastic_gradient_descent} we have
  \begin{equation*}
...
\begin{corollary}[\protect{\cite{sebbouhAlmostSureConvergence2021}}]
  ...
  Let $\gamma : \Omega \rightarrow \{1,\dots,n\}$ be a random variable defined on a probability space $(\Omega, \CF, \BP)$ and let $f_{\gamma}: \R^d \rightarrow \R$ be an $L_\gamma$-smooth and convex function. Further, $0 < \eta \leq \frac{1}{4\CL}$ and $\epsilon > 0 $. Then, for the iterates of SGD defined in equation \eqref{eq:stochastic_gradient_descent} we have 
  \begin{enumerate}[label=(\roman*)]
...
\end{proof}
  This corollary reveals an interesting property of overparameterized models for which we have $\sigma^2 = 0$. For a fixed learning rate we have linear convergence. In \autoref{sec:smdedl}, we look at the behavior of such models.
% \begin{assumption}
%   \begin{enumerate}
...
In \autoref{sec:sde_model}, we see that SGD can be approximated by a continuous-time stochastic process. We can gain insight into the behavior of SGD by analyzing this continuous-time process. However, to define this continuous-time process we must first introduce the concept of stochastic differential equations.


\section{Stochastic differential equations}
\label{sec:BackgroundSDETheory}
In this section, we introduce the concept of stochastic differential equations (SDEs). First, we define a new notion of integration: Itô integration. Then, by reformulating an ordinary differential equation (ODE) as an integral equation, we generalize ODEs to stochastic processes building on Itô integrals. Subsequently, we discuss the existence and uniqueness of solutions to SDEs. Finally, we introduce a framework for approximating solutions to SDEs numerically.
\subsection{Ordinary differential equations}
For an initial value $w_0 \in \R^d$ a system of ordinary differential equations is an equation of the form
\begin{equation}
...
\begin{theorem}[\protect{\cite[pp.~73]{ahmadTextbookOrdinaryDifferential2015}}]
  ...
  \end{equation*}
  for all $w,v \in \R^d$ and $t \in [0,T]$. Then, for every $w_0 \in \R^d$ there exists a unique solution $w:[0,T] \rightarrow \R^d$ to equation \eqref{eq:initial_value_problem}.
\end{theorem}
\begin{proof}
  The proof can be found in \protect{\cite[pp.~73]{ahmadTextbookOrdinaryDifferential2015}}.
\end{proof}
One of the simplest methods to approximate the solution of an ODE numerically is Euler's method. Li et al.\ \cite{liStochasticModifiedEquations2019} use its stochastic generalization to motivate the connection between SGD and stochastic modified differential equations (see \autoref{sec:sde_model}).
...
\end{definition}
Under a regularity assumption on the function $\mathbf{a}$ Euler's method converges to the true solution linearly.
\begin{theorem}
  \label{thm:euler_convergence}
  Let $\mathbf{a} : \R^d \times [0,T] \rightarrow \R^d$ be a continuous function satisfying a Lipschitz condition and let $w:[0,T] \rightarrow \R^d$ be the unique solution to equation \eqref{eq:initial_value_problem}. Further, let $0 \leq t_0 < t_1 < \dots < t_N = T$ be a partition of $[0,T]$ and $\{w_k\}_{k=1}^N$ be the iterates of Euler's method defined in equation \eqref{eq:ode_euler}. Then, for $\eta = \max_{k=0,\dots,N-1} t_{k+1} - t_k$ there exists a constant $C > 0$ such that
  \begin{equation*}
...
\begin{definition}[$\CV(0,T)$, \protect{\cite[pp.~5]{eAppliedStochasticAnalysis2021}}]
  Let $(\Omega, \CF, \BP)$ be a probability space and $\CF_t^B = \sigma(B_s, 0 \leq s \leq t)$ the $\sigma$-algebra generated by a Brownian motion. We define the class of functions $\CV(0,T)$ to consist of functions $f :\Omega \times [0,T] \rightarrow \R$ that satisfy the following properties:
  \begin{enumerate}[label=(\roman*)]
...
\begin{theorem}[\autocite{eAppliedStochasticAnalysis2021}]
  ...
  \begin{enumerate}[label=(\roman*)]
    ...
    \end{equation}
    \item the expected value of the Itô integral of the product $f,g$ is given by
    \begin{equation}
...
\end{multline*}
By the definition of matrix multiplication and the trace (see \autoref{sec:notation}) we have 
\begin{equation*}
    \ev{\left(\int_0^T \pmb{f}(\omega, t)\cdot d\pmb{B}_t\right)^\mathsf{T}\int_0^T \pmb{g}(\omega, t)\cdot d\pmb{B}_t}
...
\begin{theorem}[Itô's formula, \protect{\cite[pp.~232]{capassoIntroductionContinuousTimeStochastic2012}}]
  ...
  Let $f : \R^d \rightarrow \R$ be a twice differentiable function, and let $V_t = f(W_t, t)$ where $W_t$ is an Itô process defined in equation \eqref{eq:ito_process}. Then $V_t$ is also an Itô process and
  \begin{equation}
    \begin{split}
    V_t = f(W_0, 0) &+ \int_0^t \frac{\partial}{\partial t} f(W_s, s) + \nabla f(W_s, s)^\T \mathbf{a}(\omega,s)ds \\
    &+ \frac{1}{2}\int_0^t \Tr\left[\mathbf{b}(\omega,s) {H_w}_f(W_s, s) \mathbf{b}(\omega,s)^\T\right]ds \\
    &+ \int_0^t \nabla f(W_s, s)^\T \mathbf{b}(\omega,s) \cdot d\mathbf{B}_s,
    \end{split}
  \end{equation}
  where ${H_w}_f$ denotes the Hessian of $f$ with respect to its first argument.
\end{theorem}
\begin{proof}
  The proof can be found in e.g., \cite{baldiEquazioniDifferenzialiStocastiche2000}.
\end{proof}
We present the result from \cite[pp.~149]{eAppliedStochasticAnalysis2021} as a definition because in the next subsection we introduce a more general existence result from \cite{liStochasticModifiedEquations2019}.
\begin{definition}[Drift, diffusion, SDE, \protect{\cite[pp.~149]{eAppliedStochasticAnalysis2021}}]
  ...
  We call a stochastic process $\{W_t:t\in [0,T] \}$ a \emph{solution} to equation \eqref{eq:sde} if it is $\CF_t^\mathbf{B}$-measurable and the relation holds almost surely for all $t \in [0,T]$. Instead of equation \eqref{eq:sde} we use the shorthand notation
  \begin{equation*}
...
\begin{example}[Ornstein-Uhlenbeck process, see, e.g., \protect{\cite{uhlenbeckTheoryBrownianMotion1930}}]
  ...
  \end{equation}
  We see this by defining the function $f(w,t) = e^{F t}w$. Its time derivative, gradient and Hessian are given by
  \begin{equation*}
  ...
  \end{equation*}
  and finally by rearranging we obtain equation \eqref{eq:ornstein_solution_1d}.
  % 
\end{example}
...
\label{sec:sde_model}
In this section, we introduce a continuous-time model for SGD developed by Li et al.\ in \cite{liStochasticModifiedEquations2019}. First, we consider the non-deterministic case of GD and derive an ODE system to model the limiting behavior of GD following a framework found in \cite{hairerGeometricNumericalIntegration2013,smithOriginImplicitRegularization2021}. Then, we introduce a stochastic generalization of the model building on the results of \autoref{sec:BackgroundSDETheory} developed by Li et al.\ \cite{liStochasticModifiedEquations2019}. Lastly, we discuss the rigorous framework by Li et al.\ \cite{liStochasticModifiedEquations2019} to show the convergence of SGD and the continuous-time model in a weak sense and expand on some details of their proofs.
...
Recall that the iterates $\{ w_k\}_{k=1}^N$ of GD given in equation \eqref{eq:gradient_descent} for a continuously differentiable function $f : \R^d \rightarrow \R$ and a fixed learning rate $\eta > 0$ are given by
\begin{equation*}
  w_{k+1} = w_{k} - \eta \nabla f(w_{k}),
\end{equation*}
...
\begin{definition}[Gradient flow, see, e.g., \protect{\cite{santambrogioEuclideanMetricWasserstein2017}}]
  Let $f : \R^d \rightarrow \R$ be an $L$-smooth function. Then for an initial value $w_0 \in \R^d$ the system of ordinary differential equations 
  \begin{equation}
  \label{eq:gradient_flow}
  w'(t) = - \nabla f(w(t)), \quad w(0) = w_0
  \end{equation}
  is called \emph{gradient flow}.
\end{definition}
...
\begin{example}[Linear regression]
  ...
  \[
    ...
    \end{pmatrix}.
  \]
  For a learning rate $\eta > 0$ and initial values $w_{0},b_0 \in \R$ the iterates of gradient descent are given by
  \begin{equation*}
    \begin{pmatrix}
      w_{k+1} \\
      b_{k+1}
  ...
  \begin{figure}[htb]%
    ...
    \caption{Mean square loss function for the linear regression problem with $n=1000$ samples, weight $w=4$ and bias $b=1$.}%
  ...
  \begin{figure}[htb]%
    ...
    \caption{Evolution of weight $w$ and bias $b$ for the mean square loss using GD for the learning rates $\eta \in \{0.0005,0.0001,0.00005,0.00001\}$.}%
    \label{fig:learning_rates}%
  \end{figure}%
\end{example}
We reiterate some standard results from linear algebra (see, e.g., \cite[pp.~250]{romanAdvancedLinearAlgebra2010}) that we use to solve this special case of gradient flow.
...
\end{lemma}
\begin{proof}
  Because of its simplicity, we briefly show the proof. Let $A \in \R^{d \times d}$ be a symmetric, positive definite matrix. Further, let $v \in \R^d \setminus \{0\}$ be an eigenvector to the eigenvalue $\lambda \in \R$, i.e., $Av = \lambda v$. Then, we have
  \begin{equation*}
...
\begin{proof}
  ...
  \end{equation*}
  For the limiting behavior we use \autoref{lem:positive_eigen_values} and use the eigenvalue decomposition of $A = V^T \Lambda V$, where $\Lambda=\diag(\lambda_1, \dots, \lambda_d)$ and $V^TV = I_d$. Then, we have
  \begin{equation*}
    \lim_{t \rightarrow \infty} w(t) + A^{-1}b =\lim_{t \rightarrow \infty} V^Te^{\Lambda t}V(w_0 + A^{-1}b) = 0,
  \end{equation*}
...
\begin{example}[\autoref{ex:linear_regression} continued]
  Now, we apply \autoref{thm:ode_system} to the linear regression example. In \autoref{fig:scaled_weights_biases}, we observe that all curves scaled according to the learning rate follow the same trajectory $\eta$ except for the curve with learning rate $\eta = 0.0005$. This learning rate is close to the edge of stability. For example, if we increase the learning rate to $\eta = 0.001$, the GD iterates diverge. The reason why the other curves follow the same trajectory is that they all approximate the solution of the gradient flow equation. The gradient flow system is given by
  \begin{equation*}
    \begin{pmatrix}
      w'(t)      \\
      b'(t)     
  ...
  \begin{figure}[htb]
    ...
    \caption{Scaled evolution of weight $w$ and bias $b$ for the mean square loss using GD for the learning rates $\eta \in \{0.0005,0.0001,0.00005,0.00001\}$.}
...
Next, we introduce a framework taken from Hairer et al.\ \cite{hairerGeometricNumericalIntegration2013} and Smith et al.\ \cite{smithOriginImplicitRegularization2021}. We slightly modify it such that it matches the notation used for stochastic modified equations in \autoref{sec:smde_sgd} and impose some additional conditions.

\begin{definition}[Modified ordinary differential equation, \protect{\cite[pp.~337]{hairerGeometricNumericalIntegration2013}}]
  Let $\mathbf{a}: \R^d \rightarrow \R^d$ be a Lipschitz continuous function with continuous derivatives of up to order $r \in \N$ and let $w:[0,T] \rightarrow \R^d$ be the solution of the system of ordinary differential equation
  \begin{equation*}
    w'(t) = \mathbf{a}(w(t)), \quad w(0) = w_0, \quad t \in [0,T].
  \end{equation*}
  Further, for the step size $\eta > 0$ let $\{w_k\}_{k=0}^N$ be the iterates of the iterative scheme $\mathbf{\Phi}^\eta : \R^d \rightarrow \R^d$ given by
  \begin{equation*}
  ...
  \begin{equation}
    ...
    \widetilde{w}'(t) = \mathbf{a}^\eta(\widetilde{w}(t)), \quad \widetilde{w}(0) = w_0, \quad t \in [0,T],
  \end{equation}
...
\end{definition}
We note that to obtain existence and uniqueness of the solution to the modified equation, by \autoref{thm:ode_existence} the right-hand side $\mathbf{a}^\eta$ needs to be Lipschitz continuous. For example if we assume that $\mathbf{a}_0, \dots, \mathbf{a}_r$ are Lipschitz continuous, we meet the conditions of \autoref{thm:ode_existence}.
Using similar techniques as Smith et al.\ \cite{smithOriginImplicitRegularization2021} we motivate a second order modified equation:
\begin{example}
...
\end{equation*}
Solving for $\mathbf{a}_0$ and $\mathbf{a}_1$ yields 
\begin{equation*}
  \mathbf{a}_0(w) = -\nabla f(w), \quad \mathbf{a}_1(w) = - \frac{1}{2} H_f(w)\nabla f(w) = -\frac{1}{4}\nabla\norm{\nabla f(w)}^2,
\end{equation*}
where $H_f(w)$ denotes the Hessian matrix. Therefore, the second order modified differential equation for gradient flow is given by
\begin{equation}
...
In \autoref{sec:smde_sgd}, we introduce a generalization of this second order modified equation to the case of SGD developed by Li et al.\ \cite{liStochasticModifiedEquations2019}.
\end{example}
\begin{example}[\protect{\autoref{ex:linear_regression}} continued]
  ...
  \begin{equation*}
    w(t) = e^{-\frac{1}{n}X^\T Xt}(w_0 - (X^\T X)^{-1}X^\T y) + (X^\T X)^{-1}X^\T y = e^{-\frac{1}{n}X^\T Xt}(w_0 - w^\star) + w^\star.
  \end{equation*}
  ...
  \end{equation}
  We can see this by using the singular value decomposition of $X = U D V^\T$, where $U \in \R^{n \times n},V \in \R^{d \times d}$ are orthonormal matrices and $D \in \R^{n \times d}$ is a rectangular diagonal matrix. We have $X^\T X = V D^2 V^\T$ and for $A^{-1}b$ in \autoref{thm:ode_system} it follows
  \begin{align*}
...
\begin{definition}[Gradient noise process]
  ...
  \begin{equation*}
    Z_k \defeq \nabla f(w_{k}) - \nabla f_{\gamma_k}(w_{k}),
  \end{equation*}
  for $k=0,1,\dots$ \emph{gradient noise process}. For $k = 1,2, \dots$ we write 
  \begin{equation*}
...
\end{definition}
Using the decomposition \eqref{eq:sgd_decomposition} we see that SGD is a stochastic process driven by the negative gradient and the gradient noise process $\{Z_k\}_{k=0}^\infty$. Note that each iterate of the noise process has zero conditional expectation, i.e., $\ev{Z_k|w_{k}} = 0$.
...
\begin{example}[\autoref{ex:linear_regression} continued]
  ...
  Further, we use a histogram to visualize the distribution of the weight parameter $w$ for $12.5\%,\; 25\%,\; 50\%$ and $100\%$ of the total iterations in \autoref{fig:sgd_weight_histogram}. The shape of each histogram approximates a bell curve which is characteristic for a normally distributed random variable. In order to confirm this hypothesis, we make use of a quantile-quantile plot in \autoref{fig:sgd_weight_qq}.
  In this plot, we compare the empirical distribution function with the distribution function of a normally distributed random variable. We observe that empirical measurements nearly perfectly match the quantiles of a normal distribution. Therefore, we conclude that the weights are approximately normally distributed.
  \begin{figure}[htb]
    \centering
    \includegraphics[width=\textwidth]{../seminar_talk/plots/sgd_weight_bias.pdf}
    \caption{Samples of the SGD iterates for the linear regression problem and the mean of $m=10000$ independent runs for the learning rate $\eta=0.1$.}
    \label{fig:sgd_linear_fit}
  \end{figure}
  \begin{figure}[htb]
    \centering
    \includegraphics[width=\textwidth]{../seminar_talk/plots/sgd_weight_histogram.pdf}
    \caption{Histogram of the weight $w$ for the linear regression problem at 12.5\%, 25\%, 50\% and 100\% of the total iterations for a learning rate of $\eta=0.1$.}
    \label{fig:sgd_weight_histogram}
  \end{figure}
  \begin{figure}[htb]
    \centering
    \includegraphics[width=\textwidth]{../seminar_talk/plots/sgd_weight_qq.pdf}
    \caption{Quantile-quantile plot for the normal distribution and the sampled values of the weight $w$ at the last iteration of SGD for $m=10000$ independent runs.}
    \label{fig:sgd_weight_qq}
  \end{figure}
\end{example}
Following the discussion in \autoref{ex:sgd}, for the rest of this section we assume that each iterate of the gradient noise process follows a normal distribution. Li et al.\ \cite{liStochasticModifiedEquations2019} make this assumption to motivate the connection between SGD iterates and the Euler-Maruyama scheme. However, we note that for deep learning objectives experimental evidence suggest that this assumption is not valid in general. Simselki et al.\ \cite{simsekliTailIndexAnalysisStochastic2019} present experimental evidence showing that the gradient noise process is heavy-tailed and is not normally distributed. They propose a stochastic differential equation driven by a Lévy process to model the dynamics of SGD.
Following this work Zhou et al.\ \cite{zhouTheoreticallyUnderstandingWhy2020} use the Lévy driven SDE to motivate why SGD shows better generalization properties than adaptive methods in practice. In this thesis, we focus on the case of normally distributed gradient noise.
\begin{assumption}[\protect{\cite{liStochasticModifiedEquations2019}}]
  \label{as:normality}
  Let $\{w_k\}_{k=0}^\infty$ the SGD iterates defined in equation \eqref{eq:stochastic_gradient_descent} and $\{Z_k\}_{k=0}^\infty$ be the gradient noise process. Then, we assume that the iterates are normally distributed with expected value $\ev{Z_k|w_k} = 0$ and covariance matrix $\Cov \left[Z_k | w_k\right] = \Sigma(w_k)$, i.e.,
  \begin{equation*}
    Z_k \sim \CN(0, \Sigma(w_k)),
  \end{equation*}
for $k=0,1,\dots$.  
\end{assumption}
Now, in the case that the covariance matrix $\Sigma(w)$ has full rank, we write $\Sigma(w)^{1/2}$ for its symmetric and positive definite square root matrix that fulfills $\Sigma(w)^{1/2}\Sigma(w)^{1/2} = \Sigma(w)$ for $w\in \R^d$. Applying \autoref{as:normality} to the decomposition in equation \eqref{eq:sgd_decomposition} we have
\begin{equation}
...
\begin{definition}[Stochastic modified differential equation]
  ...
  \end{equation}
  modelling the continuous-time behavior of SGD is called \emph{first order stochastic modified differential equation}.
\end{definition}
Up to this point we have motivated the stochastic model of SGD using heuristic arguments. In the following, we go through the approximation results by Li et al.\ \cite{liStochasticModifiedEquations2019} and extend some details of the proof of the main convergence theorem.
...
First, Li et al.\ \cite{liStochasticModifiedEquations2019} make three assumptions that guarantee the existence of the first and second moment. We note that in the case of a risk minimization objective, where the random variable $\gamma:\Omega \rightarrow \{1,\dots,n\}$ simply selects an index, all three assumptions are satisfied.
\begin{assumption}[\protect{\cite{liStochasticModifiedEquations2019}}]
  \label{as:sde_model}
...
This equation is a stochastic generalization of the modified ordinary differential equation in \eqref{eq:modified_ode}. Li et al.\ \cite{liStochasticModifiedEquations2019} consider a first order approximation for the drift term and zeroth order approximation for the diffusion term, i.e.,
\begin{equation}
...
\begin{example}
  ...
  \begin{equation*}
    ...
    \end{cases}
  \end{equation*}
  We see that the first order weak derivative of $f$ is the $\sign$ function. Notice, that the weak derivative is not unique in a pointwise sense. In this example, we could change the value of the weak derivative at $x=0$ and the integral would not change. In general, modifications on sets of zero measure do not change the integral in equation \eqref{eq:weak_derivative}. Therefore, we understand uniqueness of the weak derivative in an almost sure sense.
\end{example}
...
Li et al.\ \cite{liStochasticModifiedEquations2019} make an important observation regarding the SMDE and SGD: the filtration $\CF^{\mathbf{B}}_{k\eta} = \sigma(B_s, 0 \leq s \leq k \eta)$ generated by Brownian motion and the filtration $\CF^\gamma_k = \sigma(\gamma_l, l = 0,1,\dots, k)$ generated by the sampling variable are independent. This implies that we cannot establish a strong convergence (see \autoref{def:strong_convergence}) of the SGD iterates and the SMDE solution. For this reason, Li et al.\ \cite{liStochasticModifiedEquations2019} consider weak convergence that implies convergence in a distributional sense for a class of functions $\mathcal{K}$ (see \autoref{def:weak_convergence}). In particular, they define the class of functions with at most polynomial growth.
...
\end{definition}
Next, we adopt some notation for the one-step error of both the SGD iterates and the solution of the SMDE:
\begin{equation}
  \label{eq:one_step_error}
  \Delta(w) \defeq w_{1}^{w,0} - w, \quad \widetilde{\Delta}(w) \defeq \widetilde{W}^{w,0}_1 - w,
\end{equation}
where $\{w_k^{w,0}: k = 0,1,\dots, N\}$ denotes the iterates of SGD defined in equation \eqref{eq:stochastic_gradient_descent} with $w_0 = w$ and $\{\widetilde{W}_k^{w,0} = W_{k\eta}: k = 0,1,\dots, N\}$ the solution of the SMDE defined in equation \eqref{eq:general_sde} with initial value $W_0 = w$ at the time steps corresponding to the SGD iterates.
Li et al.\ \cite{liStochasticModifiedEquations2019} present two results that characterize the first three moments of the one-step error for both SGD iterates and the SDE solution. In \autoref{thm:second_order}, they use these estimates to meet the assumptions of \autoref{thm:approximation} and obtain a global bound.
...
\begin{lemma}[\protect{\cite[Lemma 5]{liStochasticModifiedEquations2019}}]
  ...
  Let $\Delta(w)$ be defined as in equation \eqref{eq:one_step_error}. Suppose that for each $w \in \R^d$, $f \in G^1$. Then, for $i,j,k=1,\dots,d$ we have
  \begin{enumerate}[label=(\roman*)]
...
\begin{proof}
  ...
  \end{equation*}
  By \autoref{as:sde_model} \ref{as:bounded_gradient} and the \hyperref[thm:dominated_convergence]{dominated convergence theorem} for $i \in \{1,\dots,d\}$ it follows
  \begin{equation*}
  ...
  \begin{align*}
    \ev{\Delta_{i}(w)\Delta_{j}(w)} &= \partial_{w_i} f(w)\partial_{w_j} f(w) \eta^2 \\
    &+ \ev{(\partial_{w_i}f_{\gamma}(w)- \partial_{w_i}f(w))(\partial_{w_j}f_{\gamma}(w) - \partial_{w_j}f(w))}\eta^2 \\
...
\begin{lemma}[\protect{\cite{liStochasticModifiedEquations2019}}]
  ...
  Let $ \widetilde{\Delta}(w)$ be defined as in equation \eqref{eq:one_step_error}. Suppose that $\mathbf{a}_0, \mathbf{a}_1, \mathbf{b}_0 \in G^3$. Then, for $i,j,k = 1,\dots, d$ we have
  \begin{enumerate}[label=(\roman*)]
...
\begin{lemma}[\protect{\cite[Lemma 29]{liStochasticModifiedEquations2019}}]
  ...
  Let $\{w_k: k \geq 0 \}$ be the SGD iterations defined in equation \eqref{eq:stochastic_gradient_descent}. Suppose for some random variable $L_{\gamma} > 0$ almost surely and 
  \begin{equation*}
    \ev{L_{\gamma}^m} < \infty
  \end{equation*}
...
\begin{theorem}[\autocite{liStochasticModifiedEquations2019}]
  ...
  Further, let $T > 0$, $\eta \in (0, \min\{1,T\})$ and set $N = \lfloor T/\eta \rfloor$. Let $\{w_k:k\geq 0\}$ be the iterations defined in equation \eqref{eq:stochastic_gradient_descent}. Suppose and the following conditions are met:
  \begin{enumerate}[label=(\roman*)]
...
\begin{proof}
  ...
  \begin{enumerate}
    \item Using the smoothness and Lipschitz assumption they show existence and uniqueness of the solution to equation \eqref{eq:second_order_sde}.
  ...
  \end{enumerate}
  To show that there exists a unique stochastic process $\{W_t: t \in [0,T]\}$ satisfying equation \eqref{eq:second_order_sde} we need to show that the drift and diffusion in equation \eqref{eq:second_order_sde} satisfy the conditions of \autoref{thm:sde_existence}.

  First, we show that $\nabla f$ satisfies a Lipschitz condition. By \autoref{as:sde_model}~\ref{as:bounded_gradient} and by \autoref{thm:dominated_convergence} we have $\nabla f = \nabla \ev{f_\gamma} = \ev{\nabla f_\gamma}$. Now, let $w,v \in \R^d$. Using Jensen's inequality we have
  \begin{align*}
    \norm{\nabla f(w) - \nabla f(v)} &= \norm{\ev{\nabla f_{\gamma}(w) - \nabla f_{\gamma}(v)}} \\
    &\leq \ev{\norm{\nabla f_{\gamma}(w) - \nabla f_{\gamma}(v)}} 
    \leq \ev{L_{\gamma}}\norm{w-v}.
  ...
  \end{align*}
  From the arguments above and \autoref{lemma:linear_growth} we see that 
  \begin{equation*}
    \mathbf{a}(w) = -\nabla \left( f(w) + \frac{1}{4}\eta \norm{\nabla f(w)}^2\right), \quad \mathbf{b}(w) = \eta \Sigma(w)^{1/2}
  \end{equation*}
  fulfill both a Lipschitz and linear growth condition. Thus, $\mathbf{a}$ and $\mathbf{b}$ fulfill \autoref{as:sde_existence} and by \autoref{thm:sde_existence} equation \eqref{eq:second_order_sde} has a unique solution.
...
\begin{corollary}[\protect{\cite{liStochasticModifiedEquations2019}}]
  ...
  Assume the same conditions as in \autoref{thm:second_order}, except that we replace (i) with $f \defeq \ev{f_{\gamma}}$ is continuously differentiable, and $f \in G^3_w$.
  Define $\{W_t:t\in [0,T] \}$ as the stochastic process satisfying the SDE
  \begin{equation}
  ...
  \end{equation}
\end{corollary}
Li et al.\ \cite{liStochasticModifiedEquations2019} present an example where the solution to the first and second order approximation can be calculated analytically. We extend the calculations and add the derivation of the second order SMDE that is omitted by the authors.
% \begin{lemma}
...
\begin{equation}
  ...
  \end{split}
\end{equation}
Taking the expectation in equation \eqref{eq:expected_value_ornstein} we note that we have
\begin{equation*}
...
\begin{equation*}
  ...
  \end{split}
\end{equation*}
Now, using \autoref{lem:ito}, it follows
\begin{equation*}
...
To verify \autoref{cor:first_order}, we look at the maximum absolute difference between the expected value of the analytical solution and the mean SGD trajectories in \autoref{fig:convergence_rate}.

\begin{figure}
  \centering
  \includegraphics[width=\textwidth]{../seminar_talk/plots/sde_sgd.pdf}
  \caption{Expected loss value for the first order stochastic modified equation and mean of SGD iterates for the learning rates $\eta \in \{0.0625, 0.125, 0.25, 0.5\}$.}
  \label{fig:sde_sgd}
\end{figure}
\begin{figure}
  \centering
  \includegraphics[width=\textwidth]{../seminar_talk/plots/convergence_rate.pdf}
  \caption{Logarithmic plot of the  maximum of the absolute difference of the analytical expected value of the first order stochastic modified equation and the mean of $m=1000$ independent SGD runs.}
...
% In \autocite{liValidityModelingSGD2021a}, the authors introduce the time discrete approximation called stochastic variance amplified gradient (SVAG). This method does need evaluations of the full gradient. It is given the following recurrence relation
% \begin{equation}
...
\label{sec:smdedl}
In this section, we apply stochastic modified equations to deep learning models. First, we present the MNIST \cite{lidengMNISTDatabaseHandwritten2012} and the CIFAR-10 \cite{krizhevskyLearningMultipleLayers2009} datasets we use in our experiments. Secondly, we discuss common techniques in the construction of neural network models and look at three specific neural network architectures: fully connected neural networks \cite{aggarwalNeuralNetworksDeep2018}, AlexNet \cite{krizhevskyImageNetClassificationDeep2012} and PreResNet-32 \cite{heIdentityMappingsDeep2016}.
Thirdly, we discuss the SVAG algorithm \cite{liValidityModelingSGD2021a} which allows us to simulate the solution of the SDE model efficiently. Lastly, we compare the SGD and the SVAG iterates empirically.
\subsection{Deep learning architectures}
\label{subsec:deep_learning_architectures}
Neural network models consist of nested applications of affine and non-linear functions.   
\begin{definition}[Activation function, \protect{see, e.g., \cite[pp.~181,189,191]{goodfellowDeepLearning2016}}]
  \emph{Activation functions} are scalar functions $a:\R \rightarrow \R$ that are applied to each component of a vector $z \in \R^m$, i.e., $a(z) = (a(z_1),\dots,a(z_m))^\T$. Commonly used activation functions are:
  \begin{itemize}
    \item \emph{Hyperbolic activation}: $\tanh : \R \rightarrow (-1,1), \quad \tanh(x) = \frac{e^x - e^{-x}}{e^x + e^{-x}}$.
    \item \emph{Logistic activation}: $\sigma : \R \rightarrow (0,1), \quad \sigma(x) = \frac{1}{1+e^{-x}}$.
    \item \emph{Rectified linear unit}: $\ReLU : \R \rightarrow [0,\infty), \quad \ReLU(x) = \max\{x,0\}$.
  \end{itemize}
  A special case is the \emph{softmax activation} which maps a vector valued input to the space of probability vectors $\mathcal{P}^m \defeq \{z \in [0,1]^m: \sum_{i=1}^m z_i = 1\}$. It is given by $\softmax:\R^m \rightarrow \R^m$, $\softmax(z)$ = $e^z / \sum_{i=1}^n e^z_i$, where $e^z \defeq (e^{z_1}, \dots, e^{z_m})^\T$ is applied to each component of $z \in \R^m$.
\end{definition}
For both the MNIST \cite{lidengMNISTDatabaseHandwritten2012} and the CIFAR-10 \cite{krizhevskyLearningMultipleLayers2009} dataset we want to find a model that takes an image and predicts its corresponding label with high accuracy. This type of problem is called \emph{classification problem}. To train a model for a classification problem we define the loss function:
\begin{definition}[Cross-entropy loss, see, e.g., \protect{\cite{goodfellowDeepLearning2016}}]
  Suppose we have a classification problem with $K \in \N$ distinct classes. Let $\mathcal{P}^K$ denote the space of $K$-dimensional vectors $p \in [0,1]^K$ with $\sum_{i=1}^K p_i =1$. For a sample of features and labels $\{(x_i,y_i)\}_{i=1}^n \subset (\mathcal{X}, \{1,\dots, K\})$ of size $n\in\N$ and a model $M:\mathcal{X} \times \R^d \rightarrow \mathcal{P}^K$ the \emph{cross-entropy loss} is given by
  \begin{equation*}
    \CL(w) = -\frac{1}{n}\sum_{i=1}^n \ln(M(x_i;w)_{y_i}).
  \end{equation*}  
\end{definition}
\begin{definition}[Feed foward neural network, weight matrix, bias \protect{\cite[pp.~19]{aggarwalNeuralNetworksDeep2018}}]
  % A \emph{fully connected neural network} is a concatenation of affine function interlaced with nonlinear functions. 
  Let $W_k \in \R^{m_{k+1} \times m_k}$, $b_k \in \R^{m_{k+1} \times m_k}$ and the nonlinear activation functions $a_k : \R \rightarrow \R$ for $m_k \in \N$, $k = 1, \dots, L$, $L \in \N$ be given. The model (see \autoref{def:model}) $M : \R^{m_1} \bigtimes_{k=1}^{L} \R^{m_{k+1}\times m_{k}}\bigtimes_{k=1}^{L} \R^{m_{k}}\rightarrow \R^{m_L}$ defined by
  \begin{equation}
    \label{eq:nn}
      M(x;W_1,\dots,W_L,b_1,\dots,b_L) = z_{L+1}, \quad z_{k+1} = a_k(W_k z_k + b_k), \quad z_1 = x
  \end{equation}
  % where 
  % \begin{equation*}
  %   \label{eq:nn}
  %   z_{k+1} = a_k(W_k z_k + b_k), \quad z_1 = x
  % \end{equation*}
  for $k = 1, \dots, L$ is called \emph{feed forward neural network}. The matrices $W_1, \dots, W_{L}$ are called \emph{weights} and the vectors $b_1, \dots, b_{L}$ are called \emph{biases}. For $k=1,\dots,L+1$ we call $z_k$ in equation \eqref{eq:nn} \emph{linear layer}. The layer $z_1$ is called \emph{input layer}, the layers $z_k$ for $k=2,\dots,L$ are called \emph{hidden layers} and $z_{L+1}$ is called \emph{output layer}.
  The scalar activation functions in equation \eqref{eq:nn} are applied to each component of its input.
  In the following sections, we adopt the naming scheme of PyTorch \cite{paszkePyTorchImperativeStyle2019} version 1.13.1 to specify the structure of our models.
\end{definition}

\subsubsection{Datasets}
The MNIST \cite{lidengMNISTDatabaseHandwritten2012} dataset contains grayscale images of handwritten digits with a size of 28x28 pixels. It is made up of 60000 training images and 10000 test images.

The CIFAR-10 \cite{krizhevskyLearningMultipleLayers2009} dataset contains RGB images with a size of 34x34 pixels depicting ten different objects: airplanes, cars, birds, cats, deer, dogs, frogs, horses, ships and trucks. The dataset is made up of 50000 training and 10000 test images.
% Both these datasets are common benchmark datasets in machine learning research. 
% In our experiment, we normalize 
% In \autoref{subsec:experiments}, we investigate simulate SMDE for a two datasets: MNIST \cite{lidengMNISTDatabaseHandwritten2012} and CIFAR-10 \cite{krizhevskyLearningMultipleLayers2009}. MNIST contains images of handwritten digits and the CIFAR-10 contains images of airplanes, cars, birds, cats, deer, dogs, frogs, horses, ships and trucks. For both of these datasets the task is to train a model that correctly classefies an image.
...
In our experiments, we use a special case of a feed forward network which does not impose any special structure on its weight matrices called \emph{fully connected neural network}. In \autoref{tab:fc_architecture}, we show the network architecture of the model we used in our experiments. We use three linear layers with ReLU activation and one linear layer with softmax activation for the output layer.
\begin{table}[h!]
  \begin{center}
    \caption{Fully connected neural network for MNIST}
    \label{tab:fc_architecture}
    \begin{tabular}{l|c|c|r}
      \textbf{Layer} & \textbf{Type} & \textbf{Parameters} & \textbf{Activation}\\
      \hline
      1 & Linear & in\_features=784, out\_features=512 & ReLU \\
      2 & Linear & in\_features=512, out\_features=512 & ReLU \\
      2 & Linear & in\_features=512, out\_features=512 & ReLU \\
      3 & Linear & in\_features=512, out\_features=10 & softmax \\
    \end{tabular}
  \end{center}
\end{table}
\subsubsection{AlexNet}
AlexNet \cite{krizhevskyImageNetClassificationDeep2012} is a feed forward network neural network with some extensions that exploit the special structure of images. In deep learning tasks in which the input data $x \in \R^{m_1}$ follows grid-like structure (e.g. image data, audio data) \emph{convolutional neural networks} (CNN) employ convolution operations that make use of the locality of data. On image classification tasks CNNs perform better than fully connected neural networks (see, e.g., \cite{lecunConvolutionalNetworksImages1995}). In the following, we use the space of mappings $F:\Z^2 \rightarrow \R$ to represent 2-dimensional images. Any 2-dimensional image can be embedded in this space as a mapping $F:\Z^2 \rightarrow \R$ with finite support, i.e., $\{(i,j)\in\Z^2:F(i,j) \neq 0\}$ has a finite number of elements.
\begin{definition}[Convolution operation, see, e.g., \protect{\cite[pp.~328]{goodfellowDeepLearning2016}} ]
  A \emph{convolutional operation} for a 2-dimensional input $I : \Z^2 \rightarrow \R$ and a kernel $F : \Z^2 \rightarrow \R$ is defined by
\begin{equation*}
  O(i,j) = (F*I)(i,j) = \sum_{u,v \in \Z}I(i-u,j-v)F(u,v),
\end{equation*}
for $i,j \in \Z$.
\end{definition}
 The kernel $F$ usually has a small support. The width and height of that support is called \emph{kernel size}. There are different strategies to define the value of the convolution output $O$ at the edge of an image. A \emph{valid convolution} reduces the support of its input. On the other hand, a \emph{same convolution} pads the input image with zeros, such that output $O$ has the same support as its input $I$. CNNs also usually contain \emph{pooling layers} (see, e.g., \cite[pp.~335]{goodfellowDeepLearning2016}). A pooling layer reduces the dimensionality of its input. Two examples of pooling are \emph{average} and \emph{max pooling}. 
AlexNet \cite{krizhevskyImageNetClassificationDeep2012} consists of a block made up of convolutional and max pooling layers and a classifier consisting of linear layers with ReLU activations.
In \autoref{tab:alexnet_architecture}, we show the structure of AlexNet for the CIFAR-10 dataset we used in our experiments. 
\begin{table}[h!]
  \begin{center}
    \caption{AlexNet for CIFAR-10}
    \label{tab:alexnet_architecture}
    \begin{tabular}{l|c|c|r}
      \textbf{Layer} & \textbf{Type} & \textbf{Parameters} & \textbf{Activation}\\
      \hline
      \multirow{2}{*}{1} & Conv2d & in\_channels=3, out\_channels=64 & ReLU \\
      & & kernel\_size=4, stride=2 & \\
      2 & MaxPool2d & kernel\_size=3, stride = 2\\
      \multirow{2}{*}{3} & Conv2d & in\_channels=64, out\_channels=64  & ReLU\\
      & & kernel\_size = 4, stride=2 & \\
      4 & MaxPool2d & kernel\_size = 3, stride = 2\\
      \multirow{2}{*}{5} & Conv2d & in\_channels=64, out\_channels=64  & ReLU\\
      & & kernel\_size = 4, stride=2 & \\
      \multirow{2}{*}{6} & Conv2d & in\_channels=64, out\_channels=64  & ReLU\\
      & & kernel\_size = 4, stride=2 & \\
      \multirow{2}{*}{7} & Conv2d & in\_channels=64, out\_channels=64  & ReLU\\
      & & kernel\_size = 4, stride=2 & \\
      8 & MaxPool2d & kernel\_size=3, stride = 2\\
      \hline
      9 & Linear & in\_features=64, out\_features=448 & ReLU \\
      10 & Linear & in\_features=448, out\_features=448 & ReLU \\
      11 & Linear & in\_features=448, out\_features=10 & softmax \\
    \end{tabular}
  \end{center}
\end{table}
...
% In the end it is reduced to the output size with a pooling layer.

\subsubsection{PreResNet-32}
He et al.\ \cite{heIdentityMappingsDeep2016} introduce the PreResNet architecture that contains convolutional and pooling layers, but builds on two new innovations: Shortcut connections and batch normalization.
They observe that there is a performance drop-off for CNNs beyond a certain depth. By using shortcut connections that feed the output of previous layers directly into the next they improve the performance of large networks. In particular,  for an input $x \in \R^m$ they propose to use blocks of the form 
\begin{equation*}
  y = G(x) + x,
\end{equation*}
where $G:\R^m \rightarrow \R^m$ is called \emph{residual block} that consists of several convolutional layers with ReLU activation.
Ioffe et al.\ \cite{ioffeBatchNormalizationAccelerating2015} introduce \emph{batch normalization}. For a batch of $m$-dimensional inputs $z^{(1)},\dots,z^{(B)} \in \R^{m}$ for each component $j = 1,\dots, d$ they calculate the mean $\mu_j = \frac{1}{B}\sum_{i=1}^Bz_j^{(k)}$ and its standard deviation $\sigma_j = \sqrt{\frac{1}{n-1}\sum_{i=1}^B (z_j^{(i)} - \mu_j)^2}$, where each operation is to be understood component wise. Then they normalize each component $j=1,\dots,m$ with their respective mean and standard deviation
\begin{equation*}
  \widehat{z}^{(i)}_j = \frac{z^{(i)}_j - \mu_j}{\sigma_j}.
\end{equation*}
In our experiments we used PreResNet-32 which consists of 31 convolutional and one linear layer. In \autoref{tab:preresnet_basic_block}, we show a residual block that consists of two batch normalization and two convolutional layers. The ReLU activation function is applied before each convolution. The structure of the complete network is depicted in \autoref{tab:preresnet_architecture}.

\begin{table}[h!]
  \begin{center}
    \caption{PreResNet block with channel parameter $\in\{16,32,64\}$.}
    \label{tab:preresnet_basic_block}
    \begin{tabular}{l|c|c|r}
      \textbf{Layer} & \textbf{Type} & \textbf{Parameters} & \textbf{Activation}\\
      \hline
       & Batch2d & num\_features=3 & ReLU\\
      \multirow{3}{*}{1} & Conv2d & in\_channels $\in \{16,32,64\},$ \\
      & & out\_channels $\in \{16,32,64\},$ & \\
      & & kernel\_size=3, stride=1 & \\
       & Batch2d & num\_features=3 & ReLU\\
       \multirow{3}{*}{2} & Conv2d & in\_channels $\in \{16,32,64\},$ \\
       & & out\_channels $\in \{16,32,64\},$ & \\
       & & kernel\_size=3, stride=1 & \\
    \end{tabular}
  \end{center}
...
\begin{table}[h!]
  \begin{center}
    \caption{PreResNet-32 architecture}
    \label{tab:preresnet_architecture}
    \begin{tabular}{l|c|c|r}
      \textbf{Layer} & \textbf{Type} & \textbf{Parameters} & \textbf{Activation}\\
      \hline
      \multirow{2}{*}{1} & Conv2d & in\_channels=3, out\_channels=16 \\
      & & kernel\_size=3, stride=1 & \\
      2, 3 & PreResNetBlock & channels=16 & \\
      4, 5 & PreResNetBlock & channels=16 & \\
      6, 7 & PreResNetBlock & channels=16 & \\
      8, 9 & PreResNetBlock & channels=16 & \\
      10,11 & PreResNetBlock & channels=16 & \\
      12, 13 & PreResNetBlock & channels=32 & \\
      14, 15 & PreResNetBlock & channels=32 & \\
      16, 17 & PreResNetBlock & channels=32 & \\
      18, 19 & PreResNetBlock & channels=32 & \\
      20, 21 & PreResNetBlock & channels=32 & \\
      22, 23 & PreResNetBlock & channels=64 & \\
      24, 25 & PreResNetBlock & channels=64 & \\
      26, 27 & PreResNetBlock & channels=64 & \\
      28, 29 & PreResNetBlock & channels=64 & \\
      30, 31 & PreResNetBlock & channels=64 & \\
       & Batch2d & num\_features=64 & ReLU\\
       & AvgPool2d & kernel\_size=8 & \\
      32 & Linear & in\_features=64, out\_features=10 & softmax
    \end{tabular}
  \end{center}
\end{table}
% \begin{example}
%   Consider a classification problem, where the last layer is a linear layer with a softmax activation function, i.e.,
%   \begin{equation*}
%     M(x;W) = \text{softmax}(z) = e^{z} / \sum_{i=1}^K e^{z_i}.
%   \end{equation*}
%   For a sample $\{(x_i, y_i)\}_{i=1}^n$ of data the \emph{cross-entropy loss} is given by
%   \begin{equation*}
%     \CL(x) = -\frac{1}{n} \sum_{i=1}^n y_i \ln(e^{z_{y_i}} / \sum_{i=1}^K e^{z_i}).
%   \end{equation*}
%   We simplify this expression and obtain
%   \begin{equation*}
%     \CL(z) = -\frac{1}{n}\sum_{i=1}^n z_{y_i} - \ln(\sum_{i=1}^K e^{z_i}).
%   \end{equation*}
%   The gradient with repect to the input is given by 
%   \begin{equation*}
%     \nabla \CL(z) = -\frac{1}{n}\sum_{i=1}^n\left[ v_{y_i} - e^z/\sum_{j=1}^K e^{z_j}\right] =  \text{softmax}(z)-\frac{1}{n}\sum_{i=1}^n v_{y_i},
%   \end{equation*}
%   where $v_i = (0,\dots,0,1,0,\dots,0)^\T$ is unit vector which has a one in the $i$th entry for $i=1,\dots,K$. Now, we claim that $\CL$ is not $L$-smooth. We can show that by showing that its Hessian is unbounded. For $i,j = 1,\dots, K$ by the product rule we have
%   \begin{equation*}
%     H\CL(z)_{i,j} =
%     \frac{1}{\left(\sum_{i=1}^Ke^{z_i}\right)^2}
%     \begin{cases}
%       \sum_{i=1}^K e^{z_i+z_j} - e^{2z_i}, & \text{for } i = j, \\
%       -e^{z_i+z_j}, & \text{for } i\neq j.
%     \end{cases}
%   \end{equation*}
%   % The Frobenius norm of $H\CL(z)$ for $z = n (1,\dots,1)^\T$, $n \in \N$ is given by
%   % \begin{equation*}
%   %   \normf{H\CL(z)} = \frac{1}{\left(\sum_{i=1}^Ke^{z_i}\right)^2}
%   % \end{equation*}
% \end{example}
\subsection{Simulating stochastic modified equations}
In \autoref{sec:sde_model}, we presented the theoretical results by Li et al.\ \cite{liStochasticModifiedEquations2019} that show that for small learning rates SGD can be approximated by SMDEs. 
In particular, for certain choices of loss function (see \autoref{ex:quadratic_smde}) it is possible to derive analytical solutions to the first and second order SMDE. However, both approximation results presented in \autoref{thm:second_order} and \autoref{cor:first_order} require the loss function to be $L$-smooth. The models presented in \autoref{subsec:deep_learning_architectures} are composed of ReLU activations and batch normalization layers, which make them non-differentiable. That implies that these models are not $L$-smooth and we can neither prove existence nor uniqueness of solutions to the SMDE using the results presented.
% Additionaly, in the approximaiton \eqref{eq:first_order_convergence} and \eqref{eq:second_order_convergence} 
Still, we want to investigate whether SMDEs are a good model for deep learning objectives empirically. Recall that for an initial value $w_0 \in \R^d$ the first order SMDE for SGD is given by
% To verify this we need an efficient algorithm to simulate the solutions to the 
\begin{equation}
  d W_t = -\nabla f(W_t) dt + \sqrt{\eta}\Sigma(W_t)^{1/2}d\mathbf{B}_t, \quad W_0 = w_0, \quad t \in [0,T].
\end{equation}
In \autoref{def:euler_maruyama}, we presented the Euler-Maruyama scheme to simulate solutions to SDEs. For the first order SMDE its iterates are given by
\begin{equation}
  w_{k+1} = w_k -\eta\nabla f(w_k) + \sqrt{\eta}\Sigma(w_k)^{1/2} \Delta \mathbf{B}_k,
\end{equation} 
where $\Delta \mathbf{B}_k \sim \CN(0, \eta I_d)$ for $k=0,1,\dots$.
As we noted in \autoref{subsec:gd_sgd}, the evaluation of $\nabla f$ requires substantial computational effort. 
Additionally, explicitly computing the covariance matrix
\begin{equation*}
  \Sigma(w)= \ev{(\nabla f_{\gamma}(w) - \nabla f(w))(\nabla f_{\gamma}(w) - \nabla f(w))^\T}
\end{equation*}
and its square root matrix via a singular value decomposition has a computational cost of $\CO(d^3)$ \cite{golubMatrixComputations1996} for each iteration. Clearly, this problem is intractable for large models.

Wu et al.\ \cite{wuNoisyGradientDescent2020a} introduce multiplicative SGD which decouples the loss calculation from the sampling algorithm. This allows sampling $Z \sim \CN(0, \Sigma(w))$ without explicitly calculating $\Sigma(w)^{1/2}$ for the iterates of the Euler-Maruyama scheme.
% In the following we explain the procedure to generate these samples. Let $\pmb{f}(w) = (f_1(w), \dots, f_n(w))$ be a vector of the individual losses and let $\mathbbm{1} = (1, \dots, 1)^\T \in \R^n$ denote a vector of ones. Further, let $\widetilde{S} : \Omega \rightarrow \R^n$ be a random variable  with $\widetilde{S} \sim \CN(0, \frac{1}{bn}(I_d - \frac{1}{n}\mathbbm{1}\mathbbm{1}^\T )(I_d - \frac{1}{n}\mathbbm{1}\mathbbm{1}^\T))^\T$. Then for $S = \frac{1}{n} \mathbbm{1} + \widetilde{S}$ we have $Z = \nabla \pmb{f}(w)S \sim \CN(0, \Sigma(w))$.
% Note that the sampling of $S$ does not require calculation of the covariance matrix $\Sigma(w)$ and its square root.
However, using multiplicative SGD we still need to compute the full gradient at each iteration of the Euler-Maruyama scheme. Li et al.\ \cite{liValidityModelingSGD2021a} propose an algorithm that is based on sampled gradients and can be computed efficiently. This algorithm combines two independently sampled batches and amplifies gradient noise by a scaling factor.
\begin{definition}[Stochastic variance amplified gradient, \cite{liValidityModelingSGD2021a}]
  Let $(\Omega, \CF, \BP)$ be a probability space and $f_{\gamma} : \R^d \rightarrow \R$ be a continuously differentiable functions with $\ev{f_{\gamma}} \defeq f$. Further, let $\bar{\gamma}_k : \Omega \rightarrow \{1,\dots,n\}^2 $ with $\bar{\gamma}_k =(\gamma_{k,1}, \gamma_{k,2})$, $k=1,\dots,N$ be a sequence of i.i.d.\ random variables. We call the function $f^l_{\bar{\gamma}} : \R^d \rightarrow \R$ with
  \begin{equation*}
  ...
  \emph{stochastic variance amplified gradient function}.
    For an initial value $w_{0} \in \R^d$ the iterates defined by the recursive relation
  \begin{equation*}
    w_{k+1} = w_{k} - \frac{\eta}{l} f_{\bar{\gamma_k}}(w_{k})
  \end{equation*}
  are called \emph{stochastic variance amplified gradient} (SVAG) iterates.
\end{definition}
Li et al.\ \cite{liValidityModelingSGD2021a} construct the SVAG iterates such that for $w \in \R^d$ we have
\begin{align*}
  \ev{\nabla f^l_{\bar{\gamma}}(w)} &= \frac{1+\sqrt{2l - 1}}{2}\ev{\nabla f_{\gamma_{1}}(w)} + \frac{1-\sqrt{2l - 1}}{2}\ev{\nabla f_{\gamma_{2}}(w)} \\
...
\end{align*}
Thus, the gradients of the amplified function have the same expectation as the gradients of original objective function, but their covariance matrix scaled by a factor of $l \in \N$. We note that for the case of $l=1$ the SVAG iterates coincide with the SGD iterates.
Li et al.\ \cite{liValidityModelingSGD2021a} show that the SVAG iterates approximate the first order SMDE as $l \in \N$ grows under strong smoothness assumption. In the proof of the following theorem, they use similar techniques as presented in the proof of \autoref{thm:second_order}.
\begin{theorem}[\cite{liValidityModelingSGD2021a}]
  \label{thm:svag}
...
\end{theorem}
\begin{proof}
  The proof can be found in \cite{liValidityModelingSGD2021a}.
\end{proof}
The conditions of \autoref{thm:svag} are more restrictive than the conditions for the approximation results of the first order SMDE  presented in \autoref{cor:first_order}. Li et al.\ \cite{liValidityModelingSGD2021a} claim that by using similar mollification techniques as in the proof of \autoref{thm:second_order} the assumptions of \autoref{thm:svag} can be relaxed.

\subsection{Experiments}
\label{subsec:experiments}
In this section, we simulate the SMDE using SVAG for the three models presented in \autoref{subsec:deep_learning_architectures}: a fully connected network on MNIST \cite{lidengMNISTDatabaseHandwritten2012}, PreResNet-32 \cite{heDeepResidualLearning2016a} and AlexNet \cite{krizhevskyImageNetClassificationDeep2012} on CIFAR-10 \cite{krizhevskyLearningMultipleLayers2009}. We follow the approach by Li et al.\ \cite{liValidityModelingSGD2021a} and use the SVAG iterates for increasing values of the scaling factor $l=1,2,4,8,16$ to simulate the SMDE solution. We implement the SVAG algorithm using PyTorch \cite{paszkePyTorchImperativeStyle2019} version 1.13.0 and run our experiments on GoogleColab \cite{bisongGoogleColaboratory2019} using a standard GPU runtime. 
Each model is trained for $80l$ epochs, i.e., $80l$ passes through the entire training dataset, using a fixed learning rate decayed by $0.1$ after 80\% of training epochs for $l=1,2,4,8,16$. We note that for $l=1$ the SVAG iterates coincide with the mini-batch SGD iterates. In our experiments we investigate whether the behavior of the SGD differs from the SVAG iterates as we increase $l$ suggesting a deviation from the first order SMDE. 
We consider four metrics to compare the behavior of the SVAG iterates to the SGD iterates: the loss value and the classification accuracy for both the training and test dataset. We note that while using the SVAG iterates to simulate the SMDE solution is more efficient than using the Euler-Maruyama scheme, for the SVAG iterates with a scaling factor of $l$ we need to increase the number of epochs by a factor of $l$ compared to the standard SGD case. In the following, we say \emph{effective epochs} to mean the number of epochs divided by the scaling factor $l$.
\subsubsection{Fully connected neural network on MNIST}
For the fully connected neural network detailed in \autoref{subsec:deep_learning_architectures}, we conducted two experiments with different choices of initial learning rate $\eta$ and batch size $B$. In \autoref{fig:fc_128_012}, we show the results for initial learning rate $\eta=0.12$ and batch size $B=128$. We observe that the SGD iterates for $l=1$ and the SVAG iterates for $l=2,4,8,16$ approach the same limiting value. For the SGD iterates we see a small spike for both the training accuracy and loss at the point where the learning rate is decreased. In \autoref{fig:fc_512_048}, we show the results for $\eta=0.48$ and $B=512$. The training process is unstable for the SGD iterates and the loss value diverges. We also observe large spikes in the SVAG iterates for $l=2,4$, but the loss stabilizes for $l=8,16$.
\begin{figure}[htb]
  \centering
  \includegraphics[width=\textwidth]{../code/plots/fc128_lr012.pdf}
  \caption{Fully connected neural network on CIFAR-10 with SVAG for the scaling factors $l=1,2,4,8,16$, an initial learning rate $\eta=0.12$ and a batch size $B=128$ for $80l$ epochs for training. The case $l=1$ corresponds to the SGD iterates.}
  \label{fig:fc_128_012}
\end{figure}
\begin{figure}[htb]
  \centering
  \includegraphics[width=\textwidth]{../code/plots/fc512_lr048.pdf}
  \caption{Fully connected neural network on CIFAR-10 with SVAG for the scaling factors $l=1,2,4,8,16$, an initial learning rate $\eta=0.48$ and a batch size $B=512$ for $80l$ epochs for training. The case $l=1$ corresponds to the SGD iterates.}
  \label{fig:fc_512_048}
\end{figure}
\subsubsection{AlexNet on CIFAR-10}
For the AlexNet \cite{krizhevskyImageNetClassificationDeep2012} architecture detailed in \autoref{subsec:deep_learning_architectures} we conducted one experiment with the CIFAR-10 dataset. In \autoref{fig:alexnet_128_04}, we show the result for initial learning rate $\eta = 0.48$ and batch size $B=512$. We see that the SVAG iterates stabilize for $l=4,8,16$. In the plots for training accuracy and loss, we observe a difference in the dynamics of the SGD iterates and the SVAG iterates in the initial phase of training. The training accuracy of the SGD iterates increases more slowly than the stabilized SVAG iterates for $l=4,8,16$. This difference is also evident is the training loss.

\begin{figure}[htb]
  \centering
  \includegraphics[width=\textwidth]{../code/plots/alexnet128_lr04.pdf}
  \caption{Fully connected neural network on CIFAR-10 with SVAG for the scaling factors $l=1,2,4,8,16$, an initial learning rate $\eta=0.48$ and a batch size $B=512$ for $80l$ epochs for training. The case $l=1$ corresponds to the SGD iterates.}
  \label{fig:alexnet_128_04}
\end{figure}
\subsubsection{PreResNet on CIFAR-10}
For the PreResNet-32 \cite{heIdentityMappingsDeep2016} architecture we conducted two experiments with different choices of initial learning rate $\eta$ and batch size $B$. In \autoref{fig:preresnet_128_012}, we show loss values and classifications accuracies for both the test and training dataset for initial learning rate $\eta = 0.12$ and batch size $B=128$. We observe that the SGD trajectory mostly coincides with the SVAG iterates for $l=2,4,8,16$. The test loss shows a small difference in its limiting value.
In \autoref{fig:preresnet1024_lr096}, we have the same experimental setup up with $\eta=0.96$ and $B=1024$. We see that the training accuracy of SGD iterates grows more slowly compared to the SVAG iterates. Further, the test accuracy shows differing limiting behavior.

\begin{figure}[htb]
  \centering
  \includegraphics[width=\textwidth]{../code/plots/preresnet128_lr012.pdf}
  \caption{PreResNet-32 on CIFAR-10 with SVAG for the scaling factors $l=1$, $2$, $4$, $8$, $16$, an initial learning rate $\eta=0.12$ and a batch size $B=128$ for $80l$ epochs for training. The case $l=1$ corresponds to the SGD iterates.}
  \label{fig:preresnet_128_012}
\end{figure}
\begin{figure}[htb]
  \centering
  \includegraphics[width=\textwidth]{../code/plots/preresnet1024_lr096.pdf}
  \caption{PreResNet-32 on CIFAR-10 with SVAG for the scaling factors $l=1$, $2$, $4$, $8$, $16$, an initial learning rate $\eta=0.96$ and a batch size $B=1024$ for $80l$ epochs for training. The case $l=1$ corresponds to the SGD iterates.}
  \label{fig:preresnet1024_lr096}
\end{figure}
\subsection{Discussion}
% All in all, we saw that the SVAG trajectories coincide with the SGD iterates for the fully connected network on MNIST and show different dynamics and limiting behavior for PreResNet and AlexNet. 
In our experiments we saw that even for simple network architectures like the fully connected neural network we observe different dynamics and limiting behavior of the SVAG and the SGD iterates for large learning rates. For small learning rates the qualitative behavior was the same. However, in practice we want to choose the learning rate as large as possible to train our models efficiently. This is exactly the case when we observe differences in SVAG and SGD iterates. 

We note that from the experiments conducted we cannot conclude that the SMDE model fails in these cases. First, the loss functions do not fulfill the assumptions of the SVAG approximation result in \autoref{thm:svag}. Additionally, both the test and training accuracy are not smooth. Thus, we cannot apply \autoref{thm:svag}. Moreover, the result only implies approximation in a distributional sense. That means it is not sufficient to look at single trajectories as we did in our experiments.
All in all, we were able to reproduce the results of Li et al.\ \cite{liValidityModelingSGD2021a} for the PreResNet-32 architecture and conducted new experiments with a fully connected network and AlexNet \cite{krizhevskyImageNetClassificationDeep2012}. However, following our discussion above these results are not sufficient to determine the validity of the application of the first order SMDE to deep learning objectives.

\section{Applications of stochastic modified equations}
\label{sec:applications}
In this section, we cover an application of first order SMDE. We derive the linear scaling rule \cite{goyalAccurateLargeMinibatch2018, krizhevskyOneWeirdTrick2014} which asserts that when increasing the batch size $B \in \N$ by a factor of $m > 0$, then the dynamics of SGD are preserved if the learning rate $\eta > 0$ is scaled by the same factor. Li et al.\ \cite{liValidityModelingSGD2021a} assert that the linear scaling rule follows from the first order SMDE, but do not state how the relation can be derived. 
...
% In this thesis, we introduced gradient descent and its stochastic version. We discussed convergence analyses under strong convexity and smoothness assumptions. After that we motivated continuous-time approximations for gradient descent and derived a first and second order modified ordinary differential equation. Then we continued with stochastic gradient descent and discussed stochastic modified equations as continuous-time approximations. Then we simulated the stochastic differential equation for three deep learning models. Lastly, we looked an application of SMDE and saw how it can be used to derive the linear scaling rule.

% Capturing the dynamics of large neural networks is an open problem. SMDE is an approach that shows some promise to explain certain phenomena. However, for models that are currently applied the SMDE approximation is heuristic and current theory does not contain the interesting cases.
% There exist a lot of results for analyzing SGD in restrictive cases like convexity but how

%  do they capture the dynamics in interesting cases?
% SVAG offers an algorithm to simulate the SMDE but it is very expensive to run because the scaling factor can be large.
% It can explain some theoretical phenomena but it is not clear if it is the right approach.
% It depends on the model, the data, the learning rate, the batch size, so when does the SMDE model hold?
% We have no idea.

In this thesis, we set out to understand the theoretical properties of SGD for deep learning objectives. We started with the works of Orvieto et al.\ \cite{orvietoContinuoustimeModelsStochastic2019a} and Li et al.\ \cite{liValidityModelingSGD2021a} that discuss the properties of the SMDE model and its validity in the context of deep learning. In order to give some context to the discussion of the SMDE model, in \autoref{sec:stochastic_optimization} we discussed convergence results under restrictive convexity and smoothness assumptions. This discussion gives some insight into the behavior of SGD for overparameterized models, but the assumptions are too restrictive for deep learning objectives. In \autoref{sec:BackgroundSDETheory}, we presented some basic definitions and properties of stochastic differential equations. In \autoref{sec:sde_model}, we introduced the well-known concept of modified equations and applied it to gradient descent to motivate stochastic modified differential equations introduced by Li et al.\ \cite{liStochasticModifiedEquations2019}. Further, we presented an example by Li et al.\ \cite{liStochasticModifiedEquations2019}, in which we could solve the SMDE analytically and gain some insight about the dynamics of SGD. In \autoref{sec:smdedl}, we applied the SVAG algorithm to three neural network models and compared the results with SGD and reproduced the results by Li et al.\ and covered some new cases. Lastly, in \autoref{sec:applications}, we showed that the linear scaling rule can be derived with the first SMDEs.

All in all, SMDE are useful to model the dynamics of objective functions that fulfill $L$-smoothness and Li et al.\ \cite{liStochasticModifiedEquations2019} show a weak approximation result under this assumption. However, deep learning objective are not $L$-smooth and using the results presented in this thesis we can neither guarantee existence nor uniqueness of solutions to the SMDE. The experimental evidence presented in Li et al.\ does not conclusively show the validity of SMDE because of three factors:
\begin{enumerate}
  \item The assumptions of the SVAG convergence theorem are too restrictive for deep learning objective.
  \item Validation accuracy is not a permissible metric to compare SGD and SVAG iterates because it is not smooth.
  \item The SVAG convergence theorem shows convergence in distribution, not convergence of single samples.
\end{enumerate}
The experiments we conducted agree with the results of Li et al.\ \cite{liStochasticModifiedEquations2019}, but whether the SMDE model captures the dynamics of deep learning objectives in general, is to the best of knowledge, an open question. 
In the recent years, there is a rich body of literature building on the initial work of Li et al.\ \cite{liStochasticModifiedEquations2019} and applications like the linear scaling rule show that SMDE are a promising approach to understand the dynamics of stochastic optimization algorithms.
...
\begin{proof}
  ...
  Assuming inequality \eqref{eq:inequality} holds for $n = k-1 \geq 1$, we have
  \begin{equation*}
  ...
  \end{equation*}
\end{proof}
\begin{definition}[Mollifier, \protect{\cite{evansPartialDifferentialEquations1998}}]
  \label{def:mollifier}
...
\end{definition}
\begin{lemma}[\protect{\cite[pp. 630]{evansPartialDifferentialEquations1998}}]
...
\end{lemma}
\begin{proof}
  The proof can be found in \cite[pp. 630]{evansPartialDifferentialEquations1998}.
\end{proof}
...
\end{proof}
\begin{theorem}[Dominated convergence theorem, \protect{\cite[pp.~648]{evansPartialDifferentialEquations1998}}]
